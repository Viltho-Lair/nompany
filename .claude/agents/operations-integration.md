...
| 25/08/2026 | Open audit finding (task opened, shared with `business-logic`): input validation for the HR/Finance/Inventory/Operations modules is hand-rolled; `schema.ts` files are type-only and zod is dormant (item 14). When the zod-boundary work lands, your modules adopt the **same** `schema.ts` convention `business-logic` sets, and external payloads (carrier status, FX) get validated the same way — keep refusal bodies matching the existing goldens. | One validation convention across modules, or the choke point is not a choke point; external payloads are the least-trusted inputs and must go through it. | audit, user |
