...
| 25/08/2026 | Open audit finding (task opened): **zod is a dependency but dormant** — every module's `schema.ts` is type-only, there is no `.parse()`/`.safeParse()` anywhere in `src/**`, and item 14 is satisfied only by ~13 modules of hand-rolled `str()`/`Number.isFinite`/enum-`includes`/known-id-`Set` checks with no schema-enforced choke point. When wiring zod in, **keep the existing `{ error }` refusal bodies byte-identical** — they are pinned by goldens (`tests/goldens/**`); map zod issues back onto the current strings, never let zod's default error shape leak into a response. Coordinate the `schema.ts` convention with `operations-integration`. | The manual validators work but are unauditable as a set; the enforcement layer the schema files promise is not wired, and the golden contract makes a naive migration a build-breaker. | audit, user |
