...
async function rollover(request: Request) {
  ...
  try {
    ...
    log.error("Active-user snapshot failed", { error: err instanceof Error ? err.message : String(err) });
  }
  ...
  try {
    ...
    log.error("Year rollover email failed", { error: err instanceof Error ? err.message : String(err) });
  }
