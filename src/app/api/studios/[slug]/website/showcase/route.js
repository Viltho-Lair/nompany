import { websiteGuard, createShowcase, editShowcase, removeShowcase } from "@/lib/website";
...
export async function POST(request, ctx) {
  ...
  const result = await createShowcase(g, await body(request));
...
export async function PUT(request, ctx) {
  ...
  const result = await editShowcase(g, b.id, b);
...
export async function DELETE(request, ctx) {
  ...
  const b = await body(request);
  if (!b.id) return Response.json({ error: "missing" }, { status: 400 });
  const result = await removeShowcase(g, b.id);
