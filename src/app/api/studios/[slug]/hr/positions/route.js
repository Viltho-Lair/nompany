import { hrGuard, createPosition, editPosition, removePosition } from "@/lib/hr";
...
export async function POST(request, ctx) {
  ...
  const result = await createPosition(g, await body(request));
  if (result.error) return Response.json({ error: result.error }, { status: result.error === "duplicate" ? 409 : 400 });
  return Response.json({ ok: true, position: result.position }, { status: 201 });
}
...
export async function PUT(request, ctx) {
  ...
  const result = await editPosition(g, b.id, b);
...
export async function DELETE(request, ctx) {
  ...
  const b = await body(request);
  if (!b.id) return Response.json({ error: "missing" }, { status: 400 });
  const result = await removePosition(g, b.id);
  if (result.error) {
