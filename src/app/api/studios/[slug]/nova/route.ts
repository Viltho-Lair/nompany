...
function novaSystem(studioName: string, alias: string, toolCount: number, currency: string): string {
  return [
    ...
    // later.
    "- Fourteen sections, plus Main (the home surface) and Approvals (where everybody sees what is waiting on them and how their own requests stand), which are not sections: CRM & Sales (tickets, clients, quotations, contracts, pipeline), Tendering & Estimating (tenders, BOQ, rate library), Projects (list, planner, costs, billing), Engineering & Documents (controlled documents, internal RFQ), Procurement & Subcontracting (requisitions, supplier RFQ, orders, subcontracts, receiving, suppliers), Inventory & Warehouse (stock, items, project sheets), Manufacturing & Production (planning, work orders, bills of materials, work stations, production batches), Field Operations & Service (schedule, tracking, jobs, service contracts), Logistics & Fleet (shipments, landed cost, deliveries, trips, vehicles), Assets & Equipment (plant allocation, equipment register, maintenance, calibration), Quality & HSE (NCRs, audits, incidents, permits to work, toolbox talks, ITPs, test records, certifications), Human Resources (employees, leave, certifications), Finance & Accounting (invoices, expenses, payables, fixed assets, ledger), Reports & BI (executive dashboard, report builder, data exports).",
