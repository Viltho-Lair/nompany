...
export async function GET(request: Request, ctx: { params: Promise<Record<string, string>> }) {
  ...
  async function resolvePermissions(member: CollaboratorRef) {
...
/**
 * ONE EVENT AS THIS ROUTE READS IT. The bus hands `unknown` — a published
 * payload is JSON and nothing more — and the stream reads the same six fields
 * off it in three places, so the shape is named once here rather than cast at
 * each of them.
 */
type WireEvent = {
  id?: string;
  ...
  const visible = (e: WireEvent) => (e.scope === SCOPE.PEOPLE ? admin : viewable.has(e.sectionId));
  ...
  return sseResponse(request, async (conn) => {
    ...
    const releaseNotifs = await subscribe(CH.user(user.id), async (raw) => {
      const n = (raw || {}) as { kind?: string; studioId?: string; id?: string };
      if (!conn.open) return;
