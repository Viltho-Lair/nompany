...
// already right.
// Keyed by string because `collection` arrives off the request; the lookup is
// what decides whether this collection has a composer at all.
const COMPOSERS: Record<string, ((who: { user: unknown; slug: string }, id: string) => Promise<unknown>) | undefined> = {
...
export const GET = route(
  ...
  async ({ request, studio, sections, access, user }) => {
    ...
    if (!can(access, node.permission as PermissionKey)) return { error: "forbidden", key: node.permission };
    ...
    // it never had.
    const section = sections.find((s: Section) => s.key === node.sectionKey);
    if (!section) return { error: "no-section" };

    const compose = COMPOSERS[collection];
    const row = compose
      ? await compose({ user, slug: String(studio.slug) }, id)
