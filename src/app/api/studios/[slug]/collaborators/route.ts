...
export async function GET(request: Request, ctx: { params: Promise<Record<string, string>> }) {
  ...
  return Response.json({
    collaborators: rows.map((c, i) => ({
      ...
      overrideCount: (() => {
        const ov = (c.overrides || {}) as { allow?: unknown[]; deny?: unknown[] };
...
export async function PUT(request: Request, ctx: { params: Promise<Record<string, string>> }) {
  ...
  const incoming = (body.patch && typeof body.patch === "object"
    ? body.patch
  ...
  const patch: Record<string, unknown> = { ...assignment };
  ...
  const updated = await updateCollaborator(context.studio.id, String(body.collaboratorId), patch);
...
export async function DELETE(request: Request, ctx: { params: Promise<Record<string, string>> }) {
  ...
  await cascadeDeleteCollaborator(context.studio.id, String(targetId));
