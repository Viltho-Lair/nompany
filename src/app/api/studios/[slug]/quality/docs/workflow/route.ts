...
export const GET = route(spec, async ({ request, ...q }) => {
  ...
  return workflowFor(q, id, (permission) => can(q.access, permission as PermissionKey));
});
...
export const POST = route({ ...spec, body: true }, async ({ request, body, ...q }) => {
  ...
  return {
    ...out,
    workflow: await workflowFor(q, id, (permission) => can(q.access, permission as PermissionKey)),
  };
});
