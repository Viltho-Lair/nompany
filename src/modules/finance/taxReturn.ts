...
export async function taxReturnView(ctx: FinanceContext, query: { from?: unknown; to?: unknown }) {
  ...
  const base = text((ctx.studio as { currency?: unknown }).currency);
  const rows: TaxRow[] = [];
  ...
  for (const note of notes) {
    ...
    const { net, vat } = splitGross(note.amount, inv?.vatRate, text(inv?.currency) || base);
    rows.push({
  ...
  return {
    ...
    rate,
    ...taxReturn(rows, { from, to, currency: base }),
  };
}
