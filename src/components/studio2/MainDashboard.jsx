...
export default function MainDashboard({ slug, executive }) {
  ...
  // SWITCHED OFF, NOT LOCKED. The server lists a widget here when every
  // department it is drawn from is off; it is left out, and the grid flows on
  // without a gap — widgets are ordered by what is available, not by space.
  const hidden = new Set(executive?.hidden || []);
  ...
  // Nothing left to draw: no empty frame either.
  if (["main.activity", "main.awaiting-you", "main.event-ribbon", "main.headline-trend"].every((k) => !shows(k))) return null;
