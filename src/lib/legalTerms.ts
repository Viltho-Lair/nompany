...
// `effective` MOVED WITH 1.4, and that is §17 applied rather than skipped. Up to
// 1.3 it did not: Annex B DISCLOSED what the product already did, so the thirty
// days' notice for a material change was never engaged. 1.4 changes what an
// unpaid account keeps — closed, shut down, then deleted — and what the free
// package is, which is material by any reading, so it takes effect thirty days
// after it was published (the owner, 24/09/2026: "Update the Terms").
export const TERMS_META = {
  version: "1.4",
  effective: "24/10/2026",
  // 1.4 (24/09/2026): the owner's ladder replaces the three months' grace — an
  // unpaid account is closed at 20 days, shut down at 90 and deleted at 365;
  // only the Standard package has a free period, and it closes the day it ends.
  // 1.3 (24/09/2026): unpaid fees get three months' grace and then read-only,
  // never deletion; the free Standard package lasts three months; a studio is
  // deleted, files included, 30 days after its owner asks; billing records are
  // kept 10 years after that and then deleted — each matching the product.
...
export const TERMS_SECTIONS = [
  ...
  },
  {
    ...
    blocks: [
      ...
      { type: "p", lead: "Free tier, seats, and fair use.", text: 'A free tier (the Standard package) is available for up to four (4) Authorised Users for the first three (3) months of each studio, and requires no payment card. Only the Standard package has a free period; paid packages have none and apply once paid. When the free period ends the account is closed that day, as described in Section 5, unless a paid package has been paid for. A studio that has paid for a package cannot return to a free period. Where a plan is priced per Authorised User ("seat"), you must maintain a valid subscription for each individual accessing the Service. You may reassign a seat to a new user when a prior user no longer requires access, but seats must not be used as generic or shared logins. We may audit usage on reasonable notice to verify compliance.' },
  ...
  },
  {
    ...
    blocks: [
      ...
      { type: "p", lead: "Failed payments and suspension.", text: "If a charge fails, we may retry the payment method and will notify you. Fees are invoiced at the start of each period. If an invoice remains unpaid: (a) for the first twenty (20) days the Service remains fully available; (b) from day 20 the account is closed — you can view and export Customer Data but cannot create or change it; (c) from day 90 the account is shut down — members can no longer access it, and the owner can only pay and download Customer Data; and (d) at day 365 the account and its Customer Data are deleted in accordance with Section 10. We will send warnings thirty (30), seven (7) and one (1) days before the account is shut down and before it is deleted. Paying the outstanding amount at any time before deletion restores full access immediately. We do not charge interest on late payments; we may, however, recover reasonable, documented costs of collection where permitted by law. Suspension does not relieve you of the obligation to pay accrued fees." },
  ...
  },
  {
    ...
    blocks: [
      ...
      { type: "table", head: ["Data category", "Retention period", "Trigger"], rows: [
        ...
        ["Customer Data of an unpaid account", "Closed at 20 days (view and export only), shut down at 90 days (owner only), deleted at 365 days", "Unpaid invoice / end of the free period"],
        ["Customer Data after the owner requests deletion of a studio", "30 days, during which the Service keeps working and the request can be cancelled", "Owner's deletion request"],
        ["Customer Data at the end of those 30 days (deletion)", "Deleted, including uploaded files", "End of the 30 days"],
        ["Encrypted backups / disaster-recovery snapshots", "Rolling 35 days, then automatic expiry", "Backup cycle"],
        ["Invoices, billing and tax records", "10 years after the studio is deleted, then deleted", "Legal / tax obligation"],
  ...
  },
  {
    ...
    blocks: [
      ...
      { type: "p", lead: "Data export window.", text: "You may export Customer Data in a commonly used, machine-readable format via self-service export tools or, on request, with reasonable assistance, at any time while your account exists — including while it is closed, while it is shut down (the owner only), and during the 30 days after a deletion request." },
