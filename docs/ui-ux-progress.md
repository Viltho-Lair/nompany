...
| **1** | Token layer (primitives + semantics + spacing/elevation/radius/type) | 🟡 partial | ✅ **surface hex literals tokenised** — the two dark surfaces (`bg-white dark:bg-[#20202c]` → `--geex-surface`, `bg-slate-50 dark:bg-[#191921]` → new `--geex-inset`) resolve through tokens across 22 screens, 53 literals removed, value-identical; ~11 left where collapsing would shift a colour (documented) · ❌ the formal `primitives`→`semantics` architecture and the spacing/elevation/radius/type scales are still a larger design pass |
