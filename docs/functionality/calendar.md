...
| `src/app/super/(full)/pulse/calendar/*` | The console's screen: `page.js`, `ConnectCalendar.jsx`, `CalendarBoard.jsx` — under the Pulse shell since 10/09/2026, reached from the bottom bar. It was `(shell)/application/calendar`; the OAuth callback's `CONSOLE_CALENDAR_PATH` moved with it, or a finished grant would have returned a person to a 404 |
