...
| **One encryption key: NOMPANY_DATA_KEY** | 🔨 BUILT, NOT LIVE (17/09/2026) — model tests (field crypto, calendars) green, tsc, lint; sandbox dry run of the rekey script clean. Waits on: push, then `rekey-field-crypto.mjs` against live (two confirmations), then a commit removing the legacy branch, then deleting `FIELD_ENCRYPTION_KEY` from Vercel and `.env.local` | The owner: one key only. Stored credentials (calendar tokens, the Nova key, console MFA) are written under a subkey of `NOMPANY_DATA_KEY` and name the master that made them; old values still read until the script re-encrypts them with the key that wrote them — it refuses to write anything if one value will not open. Login-code and sign-in-state fallbacks and the device fingerprint take their own subkeys. `docs/functionality/client-encryption.md`. |
