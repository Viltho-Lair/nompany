...
| **The tender dialog promises a customer link it cannot make** | ⬜ OPEN (10/09/2026) | "Issuing body" is plain text, and its hint says *"Link it to a customer only if you already work for them"* — but the dialog has no control that sets `clientId`. The link is made only when a WON tender is handed over to Projects and the issuer is resolved into a Sales client. Either the dialog gains a customer picker or the hint goes; a hint describing a control that is not there reads as a broken screen. |
| **A tender's source is free text** | ⬜ OPEN (10/09/2026) | `source` is a 120-character text field, so "Portal", "portal" and "e-portal" are three sources and no report can group tenders by where they came from — which is the question a bid register exists to answer (*where do the tenders we win come from*). A list the studio manages in Master data, beside the other classification lists, would fix it. |
| **A tender has no owner from the day it is logged** | ⬜ OPEN (10/09/2026) | `assignedToCollaboratorId` is on the record and written by `createTender`, but the "Add a tender" dialog has no field for it — only FieldViewPanel.js StudioSales.js StudioSalesLive.js StudioTicketProfile.js touches it, and not from the create dialog — so a tender is logged with nobody responsible for chasing its deadline. |
| **Every tender is typed in by hand** | ⬜ OPEN (10/09/2026) | The dialog is the only door (`POST /tendering/tenders` → `createTender`); nothing imports a tender from a public e-procurement notice, creates one from an emailed invitation to bid, or offers one from a CRM client the invitation came from. Manual entry is the intended FIRST step — the register logs every tender from the day the studio hears of it, No Bids included — but it means a missed notice is simply absent. |
` to `
