...
| `2026-09-06-server-rendered-first-payload-*` (967) | **Phase 1 BUILT** (Tendering, `tendersView`). The fan-out uses the route itself instead of a view function per route — see the 23/09/2026 ledger row. |
...
| 16/09/2026 | A full-screen, GENERAL retail point of sale as a subsection under CRM & Sales | **FIRST SLICE BUILT (16/09/2026), on the owner's instruction:** tills, shifts with a counted close and a stored end-of-day report, scan → basket → pay, the 80 mm receipt, stock taken by expiry, and `crmSales.pos` with `discount` and `closeShift` as separate powers (`docs/functionality/pos.md`). **Still open:** returns and voids, the ledger entry per shift, the WhatsApp PDF, and every country's fiscal layer. *As first proposed:* The owner's decisions: any kind of shop, designed around no single trade; online-only (a till with no connection stops selling); walk-in customers only, so no customer list — special clients keep document invoices through Documents; the customer gets a printed receipt plus a PDF shared to WhatsApp from the till's own share menu. **The WhatsApp Business API is on hold** (no Meta Business account). The core is the same everywhere: tills, shifts with an opening float and a counted close, scan → basket → pay, stock movements, returns against the original receipt, an end-of-day report, and one ledger entry per shift. Blockers already known: items carry no barcode or pack sizes, a document carries one VAT rate rather than one per line, stock levels are summed from every movement on every read, and a sale cannot name the batch it takes. Trade rules (drug tracking, weighed items, age checks) are add-ons. Country rules are the next row. |
| 16/09/2026 | POS foundations: barcodes and packs, reads narrowed in the database, sales that take stock by FEFO | **BUILT (16/09/2026).** Items carry a barcode and packs (`barcodes.md`); `pickBatches` takes a sale's units soonest-to-expire first and never from an expired lot (`batches.md`); VAT per line arrived with the tax categories. **The repository now narrows an exact-text `where` in Postgres** (`pgReadColWhere`, `pushableWhere` in `platform/db/repo.ts`) instead of reading the whole collection and filtering in JavaScript — so checking one item's stock no longer reads every movement the studio has made. The in-memory filter still runs afterwards, so what a caller gets is unchanged; the SQL sends the key and values as bind parameters and passes the gateway's guards (a tenanted SELECT). **Not yet run against a real Postgres** — `tests/repo-pushdown-model.mjs` pins its text, and `npm test`'s integration suite is what exercises it. |
...
| **A questionnaire records what people answered** | ✅ FIXED (16/09/2026) | **It did not record anything.** `saveQuestionnaire` whitelisted six named fields and dropped the rest, so every question authored in `/super` was asked, answered, posted — and discarded, with the submit reporting success. A whitelist cannot work when the field names are authored at runtime, so the shape is **open** (`cleanAnswers`, keyed by `fieldOf`) and the **size** is closed: 200 fields, strings 2000, lists 50 × 200, finite numbers and booleans, nothing else. Beside the person's own copy there is now an ANALYSIS copy filed under the form (`q:<QuestionnaireID>:responses`), carrying `asked` — the questions **as they were put** — so a reply survives the form being reworded or a question deleted. Which form was answered is resolved from the ROUTE on the server, never from the body. `docs/functionality/questionnaires.md`. |
| **…and the response COUNT is derived, not stored** | ✅ DONE (16/09/2026) | The definition row carried `responses` and `completed` from the day it was written and **no code ever moved either** — the console's Responses column read `-` for every form, forever. The fix was not to start incrementing them: a counter beside the rows it counts is a second source of one fact, and the two part company the first time a write lands and its bump does not. Counted from the rows, one read per form on one console screen. It also removes the layering problem — `platform/db/cascade` reads `REG.questionnaires` directly to clear a deleted person's replies, the same shape it already uses for owned studios, rather than importing up into `lib/`. |
| **A question decides which question comes next** | ✅ BUILT (16/09/2026) | `lib/questionnaireLogic`, pure, read by both the live flow and the builder so the screen and the server cannot disagree. The rule lives on the question that DECIDES (`q.reveals`), because an author reading a decision question asks "what does this lead to" — the inverse shape would have meant searching every other question for a reference back. **Conditional is derived, not stored**: a question is conditional exactly when a rule names it, so deleting the rule is the safe direction. Visibility is a FIXED POINT, not one pass — A→B→C works, a rule on a hidden question does not fire, and a cycle settles with neither visible. **The respondent never sees any of it**: a hidden required question does not block the page, a page whose every question is hidden is skipped, and answers to questions no longer on screen are pruned at submit (on the screen and again on the server). `logicProblems` reports six ways a rule fails SILENTLY, in the builder, beside the rule. |
| **The console can read the answers** | ✅ BUILT (16/09/2026) | `/super/questionnaires/<id>/responses`: a per-question summary, a per-person table, and a CSV of everything — because a screen cannot anticipate the third question and a spreadsheet does not have to. The form is the spine, so a question nobody answered reads **zero** rather than being absent, and a question the form no longer asks is still reported, marked retired, under the label `asked` recorded — a report that punishes housekeeping is the thing the cost roll-up already refuses. Answered/skipped rather than a percentage: with branching most people may never have been PUT the question. **Unbaselined route** — it falls to the 300 KB default until `node scripts/bundle-budget.mjs --record` runs on a build. |
| **An author may break registration, provided somebody can see it** | ✅ BUILT (16/09/2026) | The owner's call: a form being built is allowed to be wrong, so this is **not** a refusal to save. What was unacceptable is the silence — edited into a shape that cannot produce an `intent`, the form renders, every question is answerable, the button appears, and the save is refused with nothing on screen saying why. Two halves. `registrationProblems` (pure, `lib/questionnaire`) says a clear path EXISTS, as a band across the builder, and reports five ways it does not — no `intent` question, two of them, one behind a rule nothing can fire, an optional one, or choices that cannot produce `create`/`join`. The one needing the branching engine gets `reachableIds` passed in rather than guessing. Then **Preview** proves a particular path WORKS by walking it: `/<locale>/questionnaire/preview/<id>`, super-admin only, 404 otherwise, rendering **the same component** the real survey uses — a second preview renderer would be a second reading of every rule, free to disagree about exactly what an author opened it to check. Nothing is written; the end of the run reports what would have been posted and whether it would have been accepted. The button reads "Save and preview" when dirty, because the preview reads the STORED definition and opening it dirty would show yesterday's form. `INTENTS` moved out of `identity.ts` into `lib/questionnaire` so the save and the warning cannot part company. |
| **A country's employment rules are its own** | ✅ FIXED (20/09/2026) | The owner, opening a Jordanian studio: "Social Security establishment number and UAE regulations were available in Official values even though the country selected was Jordan… studios with a set of rules and information for a specific country should not display information of anything else besides the picked one." **Official values was right** — `ssc_establishment_number` is Jordan's own, the Social Security Corporation's, and the panel only ever draws the selected country's fields. **The panel below it was wrong:** Employment rules drew all three statutory blocks for everybody, so a Jordanian company was asked for a 13-digit MoHRE establishment id and a 9-digit UAE bank routing code — identifiers from a ministry it has never dealt with — and for an end-of-service award Jordan does not grant, and could save both. A **wage protection scheme is a declared block in the country file now** (`rules.wageProtection`: its name, its ministry, which official value holds the employer id, the two identifier lengths, the currency a salary file is paid in), and **the UAE is the only country that declares one**. `employmentAppliesFor` answers the three questions from the file; the screen draws only what comes back and the settings PUT refuses the rest (`ssNotHere`/`eosNotHere`/`wpsNotHere`), so a stale form cannot store another country's scheme either. **Two defaults, because they are two answers:** a country with a pay preset has been researched, so a null block means it HAS none and is hidden; one with no preset keeps the general blocks, since hiding a scheme nobody has looked up would leave a studio unable to record what it pays into. A wage protection scheme is never a default. **The identifier lengths and the employer-id key travel with the scheme** — both were hardcoded as the UAE's (13, 9, `mohre_establishment_id`, and a constant `AED` for the file), which is the same bug one layer down: a second country's correct number would have been refused for not matching the first's. **A block a studio saved and its country does not have is shown, marked, with a Remove button, and blocks the save until it goes** — hiding it would leave figures nobody can see still feeding payroll. Verified in the sandbox: Jordan shows Social security alone; the UAE shows all three and names its own scheme; a WPS section saved under the UAE and carried into Jordan shows the amber line, refuses the save, and clears on Remove. `tests/statutory-model.mjs` (+20), `tests/official-values-model.mjs` (+6). `docs/functionality/payroll.md`, `official-values.md`. **Not built:** Saudi Arabia's and Oman's own schemes are not researched, so those studios see no such block — missing rather than wrong. |
| **A deal is measured on something — KPIs, slice 1** | ✅ DONE (20/09/2026) | The owner: a service action was a label the product knew and measured nothing by. A KPI is now DECLARED in /super → ERP settings against one service action (or against every deal) and COPIED onto a deal when its work starts — `freezeKpis`, at the one place a deal is minted, beside `freezeTemplate`. **Two kinds, because two are what the records can answer:** a milestone (done or not done, and therefore NO percentage — there is no half-signed contract) and a quantity (how many of a stage, against a target, the only kind with one). **Everything is counted off records the deal already carries**, so there is no KPI field on any record, nothing to key in and no second definition of a figure the ERP already holds. Five states, and `unknown` is a real answer rather than a nought: a KPI naming a stage the product no longer has cannot be counted, and calling that "not started" reads exactly like a company behind on its work. **It measures and never blocks** — the flow's own rule inherited whole. A deal naming no action (a project opened directly, a quotation with no ticket) still takes every declaration that measures all work, which is what `action: ""` is for; an action added mid-deal brings its KPIs measuring from THEN, and what the deal already carries keeps its own clock. Copied, never looked up: editing a declaration re-judges nothing under way, and withdrawing one leaves every deal carrying it measuring as before. **A KPI whose records the reader may not open is ABSENT** — not zeroed, not greyed — because a count is evidence that records exist, and `visibleStageTypes` asks rights AND the studio's switches, so a target on a switched-off department never appears either. The list starts EMPTY, deliberately: twenty plausible seeded targets would be twenty numbers this product invented and presented to studios as their own. `tests/kpi-model.mjs` (47). `docs/functionality/kpis.md`. **Not built:** stage-to-stage durations, studio overrides, KPIs anywhere but the deal (no widget, no report, no notification), and campaigns carrying their own. |
| **A deal's flow is this studio's, and the trades are the console's** | ✅ DONE (20/09/2026) | Three findings of the owner's, answered together. The flow's SEQUENCE ignored the section switches, so a studio with a department off was told its next step was a stage that department owns (`stagesRunning`, pure; a stage the deal already holds stays in the walk). A deal's flow was re-derived from its industry on every read although `setDealTemplate` existed to store it — only the field-service job called it — so editing an industry's default re-routed every live deal in that industry, silently; `freezeTemplate` now writes it once at the single place a deal is minted, best-effort, and older deals are left alone. And every studio saw all twenty-five trades and seven flows: `/super` → **ERP settings** owns the trades now (`REG.erpIndustries`, seeded by the code's list, revertible), a studio's Settings shows its own working set with the remainder counted, and its dialog asks only which flow its trade starts on. Not built: no warning when a section is switched off, or when a trade's flow is changed. |
| **A switched-off part's API does not answer — slice 4** | ✅ DONE (20/09/2026) | The owner: "turn off a switched-off part's API, turned back on when the functionality is on." One table (`platform/http/sectionRoutes`) maps every studio address to the part it serves — the switch, never the storage — with the addresses that belong to no switchable part exempt by name and reason; `route()` refuses `section-off` (404) before the handler, for sessions and API keys alike, and the twenty-two hand-written routes ask the same table. Settings is exempt so a studio can always switch a part back on. Reports exports, the report builder and every saved report refuse a data set whose part is off (`readDataset`, the one read door), the export list and the builder catalogue drop it, and the engagements view drops a stage whose department is off — while a destruction warning still counts through rights alone. Delivery notes and `listDeliveries` left the Inventory response, drawn by nothing since the sheet workspace. Sandbox: ~70 addresses unrefused with everything on; reads, writes and exports refused with five parts off; all restored on the next request. |
| **A widget goes when its section goes — slice 3, Reports & BI and the server** | ✅ DONE (17/09/2026) | The Reports board's eight figures each name their own part and department; a switched-off tile is dropped before its data set is read, is not counted as "hidden for want of a right", and the board groups what is left by department in sidebar order. The two tier tools now declare `anyOf` the board's parts, so every registered widget declares its sources. Every module context carries `on(key)` from all stored rows, and the per-screen routes skip reads that only a switched-off part shows (Sales tickets/clients, Quotations RFQs/list, Projects overtime/approved quotations, Field Operations permits, Inventory waybills/airlines/sheets); the Maintenance dashboard asks a switch per block; the Finance dashboard stops fetching Payables/Fixed assets while they are off. Measured in the sandbox with ten parts off and back on: query counts fell (sales 6→0, inventory 11→9, projects 10→9, field operations 5→4, maintenance 7→4) and returned on the next request. A second pass trimmed the lists that were still travelling: Inventory reads each list only for its own part (orders for Procurement → Orders, movements for Stock, items for Items or Stock, and so on), Finance reads nothing of Cash's while Cash is off and drops the margins block, HR reads only leave while Employees is off, and the Inventory dashboard stops offering cards for switched-off parts (finance 5→0, hr 3→2, inventory 11→0 queries, all restored when switched back on). |
| **Forms is a builder, not a list of settings — rebuilt 20/09/2026** | ✅ DONE (20/09/2026) | The owner, on the forms that shipped the day before: *"there is no types of questions to be added, nothing to be specified, nothing linked to create a sales ticket if needed"* — with a Google Forms editor attached as the design. **Nothing was missing, and that was the finding.** Fifteen types, branching, lead mapping, QR and embed were all built; every one of them was behind a click. The type list only appeared after pressing "Add a question" and **a question's type could never be changed once chosen**; cards were collapsed to one line until opened; and what the form DID with an answer was a single switch in another tab. A builder that hides its handles reads as a builder with no handles. So: **the editor is a full-page canvas** (`shared/studioRoute`, the till's and the planner's argument — a department sidebar beside a form's own page is two navigations competing), every card open and carrying its own **type dropdown**, duplicate · delete · Required in the footer where the hand is, drag to reorder within a section and between them, and a toolbar that floats beside the canvas on a desktop and sits along the bottom on a phone, where a floating column would cover the card being edited. **Five types added** to the owner's list — multiple-choice grid, tick box grid, time, date and time, file upload — plus a picture on a question and the rating's star/heart/thumb. **A grid is one card and several answers** (`gridField`, one key per row), which is the whole reason nothing downstream needed to learn what a grid is: the tally, the CSV and the lead's notes read it as the several questions it is, and `pagesForReport`/`askedRecord` are asserted to agree on the field names — disagreeing would have reported every grid row as a question the form no longer asks. **Sections route now** (`modules/marketing/formsFlow`, pure, walked by the page AND by the server at submit, so a form cannot refuse an answer to a question it never showed): "after section 1 → continue / go to section N / submit", and a per-answer override on a single-answer choice. A multi-select cannot jump — two answers satisfy two destinations and nothing can say which wins. A destination that stops existing is dropped at the write rather than silently falling through to the next section, and **the walk stops when a section comes round again**, because by submit the answers are fixed and the same section would resolve the same jump for ever. An answer typed into a section the walk then skipped is dropped, so it reaches neither the summary nor the notes of a lead somebody rings about. **The Sales link is rules, not a switch:** *when `<question>` is `<answer>` → raise a lead, hand it to `<person>`*, first match wins because an enquiry is one enquiry. **The old switch is read as the one rule it always meant** (`cleanSettings`, layered on read like the approval chains) so every existing form behaves identically and no script runs against a live studio. **Naming an assignee needs `crmSales.tickets.assign`, asked of whoever SAVES the rule** — invariant 5 at the door where a person chooses, since by the time it fires there is nobody to ask and it runs with the studio's authority. **File upload is the only path in this product where somebody with no account writes bytes**, so its refusals are the feature: the form must be open and must ASK that question, the type must be one it accepts, the size under the question's own limit (1/2/4 MB — the ceiling is `lib/media` reading the upload into memory under Vercel's ~4.5 MB body cap, not a product choice), and the form must be under its own total allowance, counted on the form under compare-and-set and refused with 507 when full rather than vanishing. Stored private against the studio, so reading it back is the membership check `/api/media/<id>` already makes; the answer holds media ids and nothing else; a file uploaded into a branch somebody then left is deleted at submit and its space returned. `tests/forms-model.mjs` +38. `docs/functionality/forms.md`. **Not built:** signature, picture choice and ranking; an option's own picture; a progress bar; shuffling; duplicating a whole form; rules that do anything but raise a lead; and **uploaded files are not encrypted at rest** (the answers are — invariant 18 — the bytes behind a media id are not). |
| **The public price list was the seed row** | ✅ FIXED (22/09/2026) | The owner: *"main website is behind on data displayed."* It was worse than behind — `www.nompany.com/api/pricing` served exactly ONE card and every field on it was empty, so the pricing page read **"Free · 0 USD · for up to 0 users"** with **"Save 0 %"** beneath it, under a page titled "free for teams of one to nine". Not a stale copy and not a failed read: the FX table in the same payload was current, and the home page, `/platform` and `/api/stats` all checked out (eighteen departments listed and named, figures refreshed 00:15Z that morning). **`ensureDefaultPlan()` mints a package called "Free" the first time any studio is created**, so that a studio has a plan to be pointed at — no price, no user range, nothing included — **and minted it `isPublic: true`**. `buildPricing` filters on exactly that flag and nothing else, so a bookkeeping row WAS the company's public price list, from the first studio created on the Postgres database (Redis's catalogue was deliberately not migrated) until somebody read the page. The live row `pkg_mtjx5oalubzb5c` is that seed. **Two halves, because one of them cannot reach what is already stored:** the seed is `isPublic: false` now, which stops the next one; and `offersSomething` — pure, asserted — drops a card carrying no money, no user range, no label and no contents, which stops the one already written in every studio that has ever created a plan, with nobody editing live data. **The test is "has it anything to say", never "is it free"**: a free plan with a user range, or a label, or a list of what it includes survives, because the free plan is the one this product leads with and a rule that swept it away would be worse than the defect. The board already had an honest empty state, so an unfilled catalogue now says "no packages" rather than quoting nought — the same rule the BOQ holds when it shows a dash instead of `0.00`, because nought is a price and that line has none. `tests/marketing-model.mjs` +9. **Still the owner's to do:** the real packages do not exist anywhere — names, user ranges, what each includes, the monthly figure and the yearly discount are typed at `/super` → packages, and until they are the pricing page says it has none. |
| **A widget goes when its section goes — slice 2, the department dashboards** | ✅ DONE (17/09/2026) | Ninety of the ninety-two registered widgets now declare what they are drawn from (the two left are the Reports board, which reads across departments on its own path — recorded as not built). The map follows the DATA, not the dashboard: purchase-order charts on Inventory need Procurement → Orders, permit charts on Operations need Quality & HSE → Permits, the stage charts on Sales need the Pipeline, RFI and submittal charts need their engine registers. The shell hands its section rows to the provider that already carried the tier; `useWidgetGate()` answers hidden-before-locked, and `Widget` draws nothing when hidden — so a switched-off part is never shown as an upsell. The free tiles are wrapped in `useSectionOn()` for the part each counts. Procurement and Engineering build their blocks on the server, so they now skip a switched-off part before reading it; the other dashboards hide in the browser (written down as not built). `tests/widget-sections-model.mjs` refuses a tier-only gate and an undeclared or ungated widget; `crud.mjs` proves the server side against a control studio. |
| **A widget goes when its section goes — slice 1, Main** | ✅ DONE (17/09/2026) | The owner's rule: every visual is bonded to the sections it is drawn from; a switched-off section or part switches its visuals off. Independent of the tier (what was bought) and of rights (who may read) — three gates, all must pass. Widgets are ordered by what is available, not by space. **Main was wrong at the root**: its one gate, `ctx.seen`, asked rights and never the switch, and an owner holds every right, so a studio with Projects off was told "Projects running". `seen` asks both now, which fixed the tiles, the feed, the executive widgets, *Awaiting you* and Nova's bubble in one place. Sources are **switches, not storage**: quotations and RFQs are filed under filed-only rows, so every such read names the department it is for, and `seen` throws rather than guess. Combined widgets **drop the switched-off parts** (the owner's answer) and vanish only when nothing is left; a vanished widget is not a locked teaser. `switchboard`/`widgetAvailable` in `lib/dashboardWidgets`; `tests/widget-sections-model.mjs` (19) and Main checks in `crud.mjs`. **Slice 2 — the department dashboards — is not started.** `docs/functionality/dashboards.md`. |
| **The owner chooses a studio's departments before it is created** | ✅ BUILT (17/09/2026) | The owner's instruction: a studio opened onto every department its field of work could touch, and a field of work is too blunt to decide that alone — somebody who works with data does not want to look at figures from departments they never run. **Create studio is a screen now, not a dialog**: it slides in over the account page's content column and asks, in three steps, for the company, then one yes/no question per department worded as what the company does ("Do you keep stock?"), then a review of what will be on and off. The answers are **pre-filled from the field of work** and follow it until the owner edits one. A "yes" can be narrowed to **parts** of the department (the point of sale without the rest of CRM & Sales). A department another one needs stays on and says why. Nothing is final: the review says every answer is one switch in the Sections panel. **One list for screen and route** — `studioSetupCatalogue()` — and `resolveSectionChoice` refuses anything the screen could not have sent (`sections-invalid`) or no department at all (`sections-empty`) before a slug is claimed. The screen's payload is computed on the account page, so the trade rules never ship to the browser. **`SECTION_NEEDS` gained Quotations → CRM & Sales**, where clients live; no trade default moved. A caller that sends no choice keeps the trade-only behaviour. `tests/studio-setup-model.mjs` (34) and a `crud.mjs` case against the database. `docs/functionality/sections.md`. |
| **The owner signs a stock adjustment they raised** | ✅ DONE (17/09/2026) | The owner's instruction: *"I am an Owner by default, I must have every access."* The adjustment queue refused the raiser on identity alone, owner included, and Reject sat behind the same check — so an adjustment over the limit in a one-person studio was stuck for good, visible under "Waiting for a signature" with no button anywhere. The Admin exception payroll (10/09) and bills (11/09) already carried now covers adjustments: sign what you raised, sign a later step after an earlier one, and turn it down. Non-Admins keep invariant 7; `tests/crud.mjs` asserts both halves. Bids, requisitions and payment-hold releases are unchanged. |
| **The second factor was off for everyone who had an account** | ✅ FIXED (16/09/2026) | Not a bug in the OTP code — the trust checkbox at the code step was **pre-ticked**, so every person who ever registered was silently trusted on that browser for thirty days and signing out and back in never asked for a code again. Nobody chose that default. Unticked; ticking it deliberately works exactly as before. **And "sign out everywhere" now forgets the devices too** — it dropped every session and left every browser trusted, so in precisely the case somebody reaches for it, whoever they are afraid of could sign straight back in with the password alone. Password change and reset have revoked devices all along; this was the one door of the three that did not. Ordinary sign-out is untouched, or everybody needs a code every morning. The privacy page's cookie table said `nc_dev` lasts "until you sign out of it", which was never true; it names the three things that actually end the trust. |
| **Main is a granted page** | ✅ BUILT (24/09/2026) | `main.view`, on the Access screen under Main. It was open to every member. **No existing role was given it — the owner chose "start without Main"** rather than a catch-up keyed on membership: Admin keeps it through the wildcard, every other role in every studio loses Main until somebody ticks it, and lands on its first section instead. The route refuses as well as the sidebar hiding it. `docs/functionality/sections.md`. |
| **The CSP is enforced, and Google Analytics runs on the marketing pages after consent** (26/09/2026) | ✅ BUILT | An outside scan read the report-only policy as no policy at all, which was true of what a browser refused. `next.config.mjs` now ENFORCES it, with Maps widened to Google's published host list (the report-only list would have blanked every map) and `upgrade-insecure-requests` folded in. `'unsafe-inline'` stays in `script-src` until the theme bootstrap takes a nonce. On the owner's instruction, `G-STL6EY8SEM` loads from `MarketingShell` alone — never sign-in, account, studio or `/super` — on nompany.com only, and only after Accept in a bilingual banner (`shared/marketing/consent.ts`); a footer link withdraws. Signals and ad personalisation are off. The security page and privacy policy §5/§9 (v1.3) said the opposite and were rewritten in the same change. Verified in the sandbox: enforced on every path, public and studio screens clean, banner accept/decline/withdraw in both languages. **A map was not seen loading** — no local Maps key — so open one after deploy. |
...
| **Quotations is its own department** | 🟡 BUILT (13/09/2026) — strict tsc, lint budget, `restructure`, `roles-model`, `marketing-model`, `flow-progress`, `engine-*` green; **not opened in a browser** (the database proxy needed a fresh Google sign-in) | The owner: *"a quotation is built on a technical and engineering perspective, not through sales"*; *"own section called Quotations, only Sales raises RFQs for now, but the RFQ subsection is the control unit for receiving RFQs to direct Quotations."* `quotations` with RFQs, Quotations, Live view and Settings, and the presales dashboard at its root. **Destinations, like Maintenance's service contracts:** the four old rows stay seeded as filed-only sections, so no record moves; every right keeps its name, filed under Quotations on the Access screen; old addresses resolve to the new screens; on for every trade; planted on read in every studio. Engineering & Documents is document control only. Supersedes tier 5's change (3). `docs/functionality/quotations.md`. |
| **Engineering & Documents has a dashboard of its own** | ✅ BUILT (13/09/2026) — model test, strict tsc, lint budget, `restructure`, `executive-model` green; opened in the sandbox with two RFIs and a submittal (2 open RFIs, 1 late; 1 submittal out, 1 late; the late list named both) | The owner, after Quotations left with the presales dashboard. `/engineering-docs` opens a dashboard over its six registers — waiting on you, open and late RFIs, submittals out and late, document reviews due, and a second row for transmittals, answered RFIs, BOMs and the library — with six paid widgets and the register cards kept below. **No right of its own**, deliberately: the old key opens Quotations, and every block is gated by the reader's own register right and is never read otherwise. `docs/functionality/engineering-dashboard.md`. |
| **Materials import from Excel and CSV, taken from Odoo** | 🟡 BUILT (18/09/2026) — model test green (`tests/item-import-model.mjs`, in `npm test`), tsc and strict tsc, lint within budget, opened in the sandbox end to end; not in CI, `npm test` not run | An **Import items** button beside Add item (`docs/functionality/item-import.md`). **Read in the BROWSER, not the server as this row first said**: sent to the server a file has to come back as rows, and Vercel's 4.5 MB response cap would have bounded "no row limit". .xlsx by a library-free reader in the dialog's own lazy chunk, CSV with a windows-1256 fallback for Arabic Excel. Odoo labels and technical names recognised, Product Type kept apart from category, continuation rows folded. Nothing coerced: bad numbers, unknown units and a foreign item without charges are refused by line. A likely cost/price swap must be confirmed. A SKU-less row is known by name + supplier, so a re-import does not duplicate. Update writes only the columns the file carried and needs no name. 250-row batches with a progress count, resumable because a re-sent line is counted, not written. Undo is all or nothing and refuses when any item is in use. **Two platform additions**: `repo.updateMany` and `repo.removeMany`, one event per batch, because per-row update events made every open Items screen reload the whole register and exhausted the pool. Still open: one supplier per item, opening stock, and a server-run import with the tab closed (their own rows). |
| **An item has several suppliers and its own type** | ⏸ WAITING (16/09/2026) | Odoo keeps one product with a list of suppliers (price and lead time each); nompany allows one supplier per item, so the same material from two suppliers becomes two items with split stock and split alerts. The item's type comes from the chosen supplier's list today and the field is disabled without one; it becomes the item's own, with the suppliers' lists as suggestions. |
| **Long work runs on the server and shows its progress everywhere** | ⏸ WAITING (16/09/2026) — Vercel's limits to be checked first | The owner's direction: a modern ERP keeps working when the user walks away and says what it is doing. One activity centre, with the import as its first user: batches chain on the server with no tab open, progress in the header for the starter and for admins, a notification on completion, and an import history with its counts, its error file and Undo. Running a batch twice creates nothing twice, so a broken chain simply resumes. If a serverless chain does not hold, the fallback is a Cloud Run job beside the gateway. |
| **Costs and prices can be hidden by permission** | ⏸ WAITING (16/09/2026) | Registered items carry `unitCost` and `sellPrice` to anybody who may open Inventory; no right separates them. A cost/price right, with the fields removed on the server for items, purchase orders and quotations. |
| **Stock is reserved against BOQ lines, and need date decides who gets it** | ⏸ WAITING (16/09/2026) — on the client's logistics Excel | The client's logistics team assigns materials per BOQ in Excel, so two projects can claim the same stock and nothing can rank urgency. A BOQ line links to a registered item, quantities are reserved (not only serials — to be measured first), and when stock is short the project that needs it soonest is served first. Requisitions already store `neededBy` and nothing reads it. |
| **A second stock level, and alerts that arrive** | ⏸ WAITING (16/09/2026) — on what the client means by "Pre-Order" and "Stop Quantity" | One `reorderLevel` exists and is shown on the dashboard only. A second level and a notification, not a report somebody has to open. |
| **Client balances are customer advances, and each variation has its own quotation** | ⏸ WAITING (16/09/2026) — on how Odoo records them today | The client carries unspent money to a client's next variation. Built ONLY as an auditable advance: received as an advance payment, invoiced as work is delivered, held as a liability, shown on a statement per client. Never an informal voucher — a 200k invoice for 170k of goods is what an audit flags. Variations on an open-ended project each carry their own quotation, separate from the original. |
| **Planner change history, and quotation comparison** | 🟡 BUILT (21/09/2026) — two model tests green (`plan-history-model`, `quotation-diff-model`, both in `npm test`), tsc and strict tsc, lint budget unchanged, both opened in the sandbox end to end; `npm test` not run | **The plan**: `savePlan` wrote the document whole and kept nothing, and the planner's undo dies with the page. The server now diffs the document being saved against the one stored and records it under `PLAN.history` (`docs/functionality/plan-history.md`), behind a History button. **The autosave is what shapes it** — 600ms, so saves by one person inside ten minutes fold into ONE entry keeping the first value and the last; a task added then removed leaves nothing, a session that undid itself leaves no entry, and a zoom or column toggle records nothing. Tasks match BY ID, so a dragged row moved rather than being deleted and re-added. **The quotation**: a revision opens on a copy of its predecessor and nothing said what moved. **Compare with Rev N** in the builder lists lines added/changed/removed with amounts both ways, section renames, the VAT rate and both totals (`quotations.md`). Lines match by id, then registered item, then description — a re-worded line is one line re-worded. **No route and no permission key**: the register holds every revision already, so it computes in the browser through the pure module the test asserts. Still open: comparison in Sales' own viewer, and comparing revisions that are not adjacent. |
| **Client data is sealed at rest** | ✅ LIVE (17/09/2026) — pushed; `pg-parity` passed against live Postgres with the real key; `seal-clients.mjs` sealed 19 rows in nompany, yamos and test01, the re-scan found nothing in the clear, and the clients read back through the app. Earlier status: BUILT, NOT LIVE — model test green, strict tsc, lint; the Postgres round trip is in `tests/pg-parity.mjs` and has not been run. Waits on the owner putting `NOMPANY_DATA_KEY` in Vercel and `.env.local`, then on `seal-clients.mjs` (two confirmations) | The owner: nobody reaching the database may read a client, and inside a studio clients read normally; no outside key service. Every client field, and a client's details copied onto any other record, is AES-256-GCM encrypted in `sections.ts` on the way in and opened on the way out. A master key held only in Vercel wraps a random data key per studio, so the database never holds anything that opens it; a sandbox always uses a public test key. `docs/functionality/client-encryption.md`, which lists what is not sealed yet (engagement records, the idempotency cache, notifications, generated documents, logs). |
| **An employee keeps an identity document, not ID and passport numbers** | ✅ LIVE (17/09/2026) — `identity-documents.mjs` removed the old fields (all empty) from 7 records in 6 studios and the re-scan found none left. Earlier status: BUILT — model test, tsc, lint budget; opened in the sandbox: Passport chosen, Save blocked until an expiry was picked, saved, and the expiring list read "Passport · in 13 days"; the server refused a missing expiry, an unknown type and a foreign or unknown picture. The picture upload itself was NOT exercised — no Blob token on this machine. `identity-documents.mjs` waits on two confirmations | The owner: the encrypted ID and passport numbers go, from the code and from every record. In their place a document type (six, translated), an expiry required once a type is chosen, and an optional picture uploaded privately to this studio. The picture is shown to, and changed only by, holders of `hr.employees.salary` — which also closes audit finding M-9. `FIELD_ENCRYPTION_KEY` STAYS: calendar tokens, the Nova key, console MFA and IP hashing still use it. The script carries each person's ID or passport expiry over (the sooner one when both) and deletes the six old fields, after an export. |
| **One encryption key: NOMPANY_DATA_KEY** | ✅ DONE (17/09/2026) — the console action converted 2 documents, a live scan found nothing left, and the action, the legacy read path and `FIELD_ENCRYPTION_KEY` (code and `.env.local`; Vercel by the owner) were removed. Earlier status: — model tests (field crypto, calendars) green, tsc, lint; sandbox dry run of the rekey script clean. PUSHED 17/09/2026. The live dry run found 3 values (the Nova key, the console Google tokens) that the LOCAL `FIELD_ENCRYPTION_KEY` cannot open, and the live one is unreadable from Vercel — so the conversion became a console action (Settings → Security → Encryption key), tested end to end in the sandbox. Waits on: that action being run, then a commit removing the legacy branch, then deleting `FIELD_ENCRYPTION_KEY` from Vercel and `.env.local` | The owner: one key only. Stored credentials (calendar tokens, the Nova key, console MFA) are written under a subkey of `NOMPANY_DATA_KEY` and name the master that made them; old values still read until the script re-encrypts them with the key that wrote them — it refuses to write anything if one value will not open. Login-code and sign-in-state fallbacks and the device fingerprint take their own subkeys. `docs/functionality/client-encryption.md`. |
| **Point of Sale is its own department, with a sales list, shift history and downloads** | ✅ BUILT (17/09/2026) — pos-model, restructure, roles, studio-setup, marketing, numbering green; tsc, lint; opened in the sandbox with two sales (dashboard totals and the full items list, the Sales list with its filters, a receipt reprint, all three downloads and a ticked-receipt download, shift history with expected cash, Settings, the old till address, the Access screen) | The owner, 17/09/2026: sales from the till had nowhere to be seen. The seventeenth department: a dashboard (takings, best sellers, everything sold), the till, a Sales list (every receipt with its cashier — the person signed in — filters, reprint), shift history (stored end-of-day reports), and Settings moved off the till. Downloads by day (default today), week, month, quarter, half-year or year, and items sold by period, cashier and chosen receipts, as totals or every line. Rights: `pos.dashboard`, `pos.sales` (+`export`), `pos.shifts`, `pos.settings`; the till keeps `crmSales.pos`; existing roles catch up. Records stay under `crm-sales-pos`, now filed-only. `docs/functionality/pos.md`. Stock alerts and removing packs follow. |
| **Stock alerts at the reorder level** | ✅ BUILT (17/09/2026) — stock-alerts-model, roles, restructure, access, notices green; tsc, lint; in the sandbox an item at 14 with a level of 10 sold to 11 (no alert) then 9 (one alert, "Stock at reorder level"), a further sale sent nothing, and both dashboards listed it | The owner: alerts about stock against the reorder level, managed in Access and held by the owner by default, and a list of items close to or under the level. `inventory.stock.alerts` (an extra on Stock, seeded to nobody); a bell notification the moment an item falls to its level, from any path that takes stock; a "Stock to reorder" list (under, then within 20% above) on the Inventory and Point of Sale dashboards. `docs/functionality/stock-alerts.md`. |
| **Items no longer have packs** | ✅ BUILT (17/09/2026) — barcode-model, pos-model, tsc, lint; sandbox dry run of the clean-up clean. `remove-packs.mjs` against live waits on two confirmations | The owner: packs are not needed; the item's unit, which a studio names itself, says how it is sold. Removed from the item form, the item service and schema, barcode scanning (one code per item), the till's basket and its arithmetic, and the dictionary. Past receipts keep what they printed. Saving an item drops any packs it carries; the script clears the rest after an export. `docs/functionality/barcodes.md`. |
| **Human Resources is five sub-sections, not one screen with seven tabs** | ✅ BUILT (17/09/2026) — restructure, roles-model, access, tsc, lint | The owner's instruction, against the HR implementation plan. Employees · Lifecycle & contracts · Time & attendance · Leave · Payroll, each a sidebar child with its own screen. **No row moved and no right was renamed**: attendance, pay records, payroll runs and leave stay filed under `hr-employees` and the HR root, and the four existing areas each answer for their own sub-section in `SECTION_AREAS` instead of all hanging off Employees — so a payroll clerk holding `hr.payroll` alone opens Payroll rather than opening Employees and finding six tabs they may not use. Manpower stayed a tab: it answers to `hr.employees`, so a section of its own would hold no separately grantable right (invariant 16). |
| **The employment lifecycle: contracts, probation, notice, exit and a final settlement** | ✅ BUILT (17/09/2026) — hr-lifecycle-model (70 assertions), restructure, roles-model, tsc, lint | `docs/functionality/lifecycle.md`. HR could describe a person and not their EMPLOYMENT — no contract, no probation, no notice, no leaving date — so `endOfService`, correct and researched per country since 11/09, computed an award **nothing could trigger**. Two collections (`employmentContracts`, `lifecycleEvents`) under a new `hr-lifecycle` section, six states, an append-only event log, and `employmentStatus` stored on the collaborator row so payroll and leave never replay it. An amendment SUPERSEDES rather than edits, and `contractAt` answers what somebody was on at a date. Catalogue 202 → 206 (`hr.lifecycle` view/create/edit + `offboard`). |
| **Employment rules are an effective-dated country pack** | ✅ BUILT (17/09/2026) | The owner's choice over a runtime pack loader. `modules/hr/packs/employment.ts`: probation, notice and the contract types each country recognises, keyed by the date ASKED ABOUT rather than today — a probation end is computed from a contract months after signing, and one current figure would silently re-date every historical contract the day a law changed. Jordan, Saudi Arabia and the UAE, each citing its article; **the UAE offers no permanent contract** (Decree-Law 33/2021 art. 8), which is the difference that proves the pack decides something. An unresearched country gets a fallback, never another country's law. `COUNTRY_PRESETS` (the PAY half) is untouched and stays a confirm-by-hand preset. |
| **A settlement is money, so it answers to a pay right** | ✅ BUILT (17/09/2026) | An end-of-service award is a multiple of a monthly wage, so a reader shown the award has been shown the wage. `hr.lifecycle.offboard` books the exit; `hr.employees.salary` (or `hr.payroll.view`) computes and stores the figures. An offboarder without either records the exit, the service years and the leave days with no amounts, and the screen says why. Notice in lieu is SIGNED — paid to somebody dismissed, owed by somebody who walked out — and `complete` travels with the total the way `boqTotals` does. |
| **Payroll and leave read the employment** | ✅ BUILT (18/09/2026) — payroll-model, leave-model, hr-lifecycle-model, npm test, lint budget 102 → 101 | The join the lifecycle was stored for. `employedBetween` (pure, one place) answers whether an employment was live in a window, and three readers ask it: `prepareRun` decides who is in a run from the EMPLOYMENT rather than from the pay record — every pay record used to become a payslip, so somebody who left in March was paid in full in April and every month after; `allowanceFor` pro-rates the LEAVING year as it always pro-rated the joining one, ending a leave liability that grew for ever on people who had gone; and `headcount` counts the employed, so a studio's figure can go down. |
...
| **Point of Sale: returns, repeat customers, discounts — six slices** | ✅ BUILT (18/09/2026) — units-model, pos-model, barcode128-model (ZXing decoded every character), phone-model, pos-return-model, credit-note-model; each slice opened in the sandbox | Built as planned, with three things found on the way: an invoice line naming an item fills its name, price and tax from it; **Finance → Cash gained a Credit notes tab** (the routes existed and no screen called them, so a return's draft note had nowhere to be issued); and **an issued credit note now comes off what the invoice still owes** (`netOfCredits` had described it and nothing read it). The owner may sign what they themselves asked for, the Admin exception bills and adjustments carry. Not seen printed: the barcode on a Documents page (publishing a layout needs a second person). The owner's decisions: (1) **each built-in unit can be switched off** in Master data → Units; an item already measured in it keeps it. (2) **A discount field at the till**, per line and on the whole basket, % or amount, behind `crmSales.pos.discount`, with a studio cap on the % a cashier may give; the receipt keeps the regular price and each discount per line, because returns and credit notes read it. (3) **A Code 128 barcode** of the number on the till receipt and on printed documents, for lookup by scanner; the QR waits (Open decisions). (4) **A phone number at the till registers a repeat customer** as a client with no name until somebody edits it; found again by a keyed hash of the number (a purpose subkey of `NOMPANY_DATA_KEY`), since every client field is sealed. (5) **Returns, a sub-section of Point of Sale**: against a till receipt found by its barcode; **every return waits for a manager's signature** (a fifth approval chain), and an approved return puts its units back into stock; the cashier chooses cash, card or transfer for the refund; a line cannot be returned twice. (6) **Invoice lines may name a registered item**, and a return against a Documents invoice restocks those lines and issues a credit note for what is refunded; free-text lines are refunded only. Slices in this order, one commit each. |
| **Finance is measured against the owner's *Finance Section Implementation Plan*, and deepened in six steps** | 🟡 ALL SIX STEPS BUILT (18/09/2026; `npm test`, the build and parity not run) — STEP 1 BUILT (18/09/2026) — model tests (`statements-model`, `withholding-model`, `fx-model`, `asset-posting-model`, `finance-posting`, `periods-model`) green, tsc and lint clean; `npm test` and the sandbox not run. Commits `76e21ef1` statements keep the third decimal · `37226986` input VAT on 1400 · `35b551de` withheld tax leaves AR on settlement (1300) · `f289b22a` foreign bills converted at a frozen rate, payments book Exchange Differences (5800) · `1cf6a634` fixed assets posted: acquisition by a stated source, a previewed monthly depreciation run, disposal with gain/loss (5400, 4900). Money storage is an open decision below. **Step 1 verified in the sandbox**, which found one defect: the register reckoned a disposal's gain at today rather than the disposal date (`5338b79b`). 🟡 **STEP 2 BUILT (18/09/2026) — verified in the sandbox, model tests green, tsc and lint clean; `npm test` not run.** `57f200f9` the Ledger screen writes: manual entry, a reversal that asks why, a chart editor (rules in `ledger.md`; `finance.ledger.post`, no new key) · `ee8c44e6` several money accounts (an asset marked `cash`, plus 1000/1010), payments and expenses name one, transfers between them, reconciliation per account, the forecast from their total · `13efff3a` a cheque settles the invoice or bill it names: the payment posts to Cheques Receivable/Payable (1150/2050), clearing moves it to the money account, a bounce marks the payment `bounced` and the document owes again. The credit-note screen was built by another session (`9e635c3e`). **Deal-level `payments` are left alone**: API-only, no screen, and the same money an invoice payment posts — retire or link them when a screen needs them. 🟡 **STEP 3 BUILT (18/09/2026) — every screen opened in the sandbox; model tests (`roles-model`, `widget-sections-model`, `reports-model`, `customer-documents`, `access.test`, finance models) green, tsc clean, lint 0 errors; `npm test` not run, and the database suites' fixtures were moved to the new rights without being run.** Finance is eight sub-sections, no row moved, and **a right per sub-section — the owner's choice over reusing the old rights**: `finance.receivables`, `finance.expenses`, `finance.tax`, `finance.reports` (catalogue 235 → 248 with three from other work), caught up from `finance.cash` and `finance.ledger` for every existing role. Details and what does not catch up (individual overrides) in `sections.md`. Found on the way: the `/finance` read checked no right at all, and the withheld-tax-to-claim list was computed and shown nowhere — Tax shows it now. 🟡 **STEP 4 BUILT (18/09/2026) — the owner's scope: withholding on bills, VAT filing, zakat now, e-invoicing framework only. Every piece exercised in the sandbox (a Saudi studio); model tests (`withholding-model`, `zakat-model`, `einvoice-model`, `finance-setup-model`, `roles-model`) green, tsc and lint clean; `npm test` not run.** `47214afb` a bill withholds from its supplier by the same rule an invoice's client does (2150), both forms offer the rule (neither did — withholding was API-only), the Tax screen records certificates on both sides · `491c2541` a VAT return is filed once (`finance.tax.file`, caught up from `finance.ledger.close`), settled from the LEDGER's own movement into VAT Due (2160) with any document/ledger difference shown, and paid from a money account · `8d69d521` a zakat worksheet for a studio whose country levies it (`rules.zakat`, Saudi Arabia): the ledger's equity, fixed assets and profit plus the accountant's adjustments, the 2024 regulation's floor and cap, the rate prorated to the fiscal year's days, provisioned (5950/2170) and paid — **and the chart can no longer seed a default account twice**: the Tax screen's parallel reads had duplicated 5950 and 2170 in the sandbox, so default accounts are now seeded under a fixed id per code (the primary key refuses a second copy) and a code held twice resolves to the oldest; every one of the eleven accounts steps 1–4 added would otherwise have risked duplicating on every live studio's first busy read · e-invoicing framework (`einvoicing.md`): `rules.einvoice` for Saudi Arabia and Jordan, an adapter contract with an EMPTY registry, the Tax screen's queue of issued invoices still to reach the authority, the setup notice saying nompany does not submit yet, and a submit that refuses `no-adapter` without writing. 🟡 **STEP 5 BUILT (18/09/2026) — model tests (`statements-model`, `credit-model`, `payment-run-model`, `expense-claims-model`, `reconciliation-model`, `budgets-model`, `periods-model`, `roles-model`) green, tsc and lint clean; every screen opened in the sandbox; `npm test` not run.** `ac963859` a cash flow statement (direct, off the journal; Reports → Cash flow) and a year-end close into 3900 Retained Earnings that locks the twelve months and reopens with a reason (Ledger → Periods; the P&L leaves closing entries out) · `80eaf891` customer credit limits and holds checked when an invoice is ISSUED, with a recorded override, and payment reminders at the studio's day levels (Receivables → Credit, → Reminders; nompany prints the letter, it does not send it) — and **Cash & Bank's Treasury tab had rendered nothing since the step-3 split**, fixed; Finance settings' read now asks for its right · `55a4a477` payment runs: approved bills due by a date paid together, each through the bill's own pay door · `17d82bb9` expense claims (a new right, `finance.claims`; nobody approves their own) and staff advances drawn down by claims (1250 Staff Advances, 2210 Staff Claims Payable) · `12489311` bank CSV import that skips what is already on the statement, and match rules that post and pair the charges the books never hold · `a1ce30e8` **Budgets, a ninth Finance sub-section** (`finance.budgets`): a year per account, for the studio or one project, deal, cost code or department, against the P&L's own actuals · `c6b45386` the close checklist: five checks the books answer and the studio's own month-end tasks, never a gate. Catalogue measured 256 over 81 areas. **Not exercised in the sandbox:** paying a bill through a run (bill approval now asks for the owner's signing PIN, not entered) and approving a claim (the sandbox has one member). 🟡 **STEP 6 BUILT EXCEPT MULTI-ENTITY (18/09/2026) — model tests (`schedules-model`, `leases-model`, `allocations-model`, `budgets-model`, `statements-model`) green, tsc and lint clean; every screen opened and every run posted in the sandbox; `npm test` not run.** `be489670` deferral schedules: revenue earned over time (IFRS 15) and prepaid costs, deferred on their day into 2300 Deferred Revenue / 1450 Prepaid Expenses and recognised month by month from a previewed run (Ledger → Schedules) — shares split in the currency's minor units, which caught twelve shares of 1,000 riyals posting 999.97 · `9c9f9e7d` IFRS 16 leases (Fixed assets → Leases): the right of use (1600) and the liability (2500) at present value, then one entry a month for depreciation (5410/1610), interest (5810) and the payment, the liability running to exactly nought; twelve months or less refused as the short-term exemption · `7a25e15c` allocations (Ledger → Allocations): an account's unowned part shared across projects, deals, cost codes or departments by fixed percentages or by what each earned, same account both sides so the P&L total never moves. **Multi-entity: the owner chose studios, grouped (18/09/2026)** over entities inside one studio — each company stays its own studio and a group reads their books as one (`groups.md`): the owner alone groups studios they own; the consolidation opens every member through `studioContext` and needs `finance.reports.view` in each, refusing without naming a member the reader cannot open; P&L and balance sheet translated at one current rate per member, added by account code, 1170/2070 intercompany balances eliminated and any disagreement named. Verified in the sandbox with one member only (the sandbox account owns one studio). |
| **E-invoicing adapters, one per country** | 🟡 **CHANGED TO: PREPARE, NEVER SUBMIT (26/09/2026).** The owner: *"our job is to prepare companies to submit their invoicing on the official channels not through us … we must not interact with official channels at all."* An adapter now PREPARES each invoice's official file; the company downloads it, submits it itself and records the authority's answer. **Removed the same day:** Jordan's JoFotara submission (on `main` since 22/09 — the API call, the Send button, the daily retry cron and the Studio settings credentials panel) and Saudi Arabia's ZATCA onboarding, signing and reporting (built 25/09, never committed). Jordan's income source sequence and invoice code moved to Official values, because they are printed inside the file. **Stored credentials were NOT deleted**: nothing reads them, the export strips them, and deleting live data waits on the owner's two confirmations (invariant 17). The work list below is rewritten to match. | The framework (`einvoicing.md`) and two adapters, Jordan (`jofotara`) and Saudi Arabia (`zatca`), each keyed by the name the country's definition declares, each with `prepare` and nothing else — `tests/einvoice-model.mjs` asserts that, and that no e-invoicing module calls `fetch` and no file names an authority's API host. **Jordan:** ISTD's UBL, with the eight corrections ISTD's own guide made to a tutorial; the QR is JoFotara's, pasted back by the studio. **Saudi Arabia:** ZATCA's UBL, unsigned, with the counter and hash chain and the QR's five seller tags; the phase-2 stamp belongs to the company's own certified solution. **Found on the way (25/09/2026):** Jordan's `cac`/`cbc` namespaces lacked UBL 2.1's `-2` suffix, and Jordan's adapter read its credentials from a key nothing wrote. The first is fixed; the second went with the credentials. Build a country's adapter when a studio there needs one (the 16/09 rule). | The plan (18/09/2026: nine sub-sections, one posting engine, Essential / Standard / Advanced tiers, country packages) compared with the code. **The engine is further along than "basic" suggests**: a real double-entry ledger, no edit after posting, mirror reversals, month locks, automatic posting of invoices, expenses, bills, both payment kinds, credit notes and payroll, a bill approval chain and the three-way-match payment hold. **What is wrong is that the books are incomplete and the sub-sections are one level deep.** Found: (1) money that moves and never reaches the ledger — depreciation, asset purchases and disposals, the withheld part of an invoice (its receivable never clears), cheques and deal-level payments; (2) a foreign-currency bill posts its raw amount into the studio-currency book (`money.md`); (3) `statements.ts` rounds at two places, so a JOD/KWD/BHD/OMR studio's P&L and balance sheet lose the third decimal the ledger kept; (4) input and output VAT share account 2100; (5) the Ledger screen only reads (no manual entry, no reversal, no chart editor), one bank account, no cash-flow statement, no year-end; (6) four of the plan's nine sub-sections do not exist (Tax, Budgets, Reporting & Close, Entities), and invoices sit under **Cash** where the plan has Receivables. **The six steps, in the owner's order:** 1 complete the books (post what moves, convert FX, fix the statements, split VAT) · 2 make the ledger usable (chart editor, manual entry, reversal button, several bank/cash accounts, transfers, credit-note screen, cheques LINKED to the invoice or bill they settle) · 3 re-file the sub-sections the Quotations/POS way (new nav keys, rows stay where they are filed) · 4 the country essentials, per the studio's country (e-invoicing adapters such as ZATCA, withholding on bills, VAT return filing, zakat where it applies) · 5 Standard (payment runs, expense claims, dunning, credit limits, statement import and match rules, budgets on the four existing line dimensions, close checklist, year-end, cash flow) · 6 Advanced (multi-entity, IFRS 15/16, allocations). **Where the plan is adapted rather than followed:** its `/api/finance/*` + `X-Company-Id` shape contradicts invariant 2 and the studio route wrapper, so its entities are kept and its routes are not; approvals extend `platform/approval` rather than moving to the Task system; multi-entity needs a company layer INSIDE a studio, which is its own design. **THERE IS NO HOME MARKET — the owner, 18/09/2026: "the studio sets the home market when the country is selected."** The plan built a Saudi package first and `withholding.md` refused ZATCA because "this product is based in Jordan"; both assumed the product has a country, and it does not. Step 4 is therefore **country essentials, per the studio's country**: e-invoicing adapters (ZATCA for a Saudi studio, and each country's own after), withholding on bills, VAT return filing, zakat where the country has it — each living in the country's definition (`shared/compliance`) and switched on by a studio choosing it. **AND IMPORTANT SETUP IS ANNOTATED FOR USERS** (same instruction): the Finance screens say what the studio has not set up — country, currency, a VAT rate to check, and the official values its country requires on its documents — why each matters, and where to set it (`modules/finance/setup.ts`, `FinanceSetupNotice`). **Cheques and deal payments are NOT posted in step 1**: each is the same money an invoice or bill payment already posts, so posting them unlinked would count it twice — they post once they name the document they settle (step 2). |
| **Country compliance: a country's official values apply by themselves once the Owner picks the country** | ✅ SLICES A–D BUILT (18/09/2026) — official-values-model and customer-documents green, tsc and strict tsc clean, eslint clean, `npm test` green; the panel and the receipt heading checked in the sandbox, a printed layout not. B: quotations, invoices and POS receipts print the official values their country marks for them, an empty one prints nothing, and a legal row repeating one is dropped. C (the owner chose the payslip over HR letters, which do not exist): one person's payslip prints from a run, carrying the official values marked `payslip`. D: the tax profiles, employment packs and pay presets moved out of code into the country files' `rules` (figures unchanged, their tests unchanged and green), seventeen rules-only files for the countries researched for tax alone, and the UAE's WPS employer ID became an official value | `docs/functionality/official-values.md`. Phase 1 researched seven countries (SA, AE, JO, EG, US federal, GB, DE) field by field with sources. Slice A is the foundation: one JSON definition per country, a validator shared by screen and server (formats plus a closed set of checksums), an **Official values** panel in Studio settings showing only the selected country's fields, an all-or-nothing save with an append-only change history, and one resolver that returns a value only when **selected, filled, applicable and valid for the current country** — otherwise an empty string, never another country's value. Adding a country is adding a file. Still open: HR letters (no employee-bound document exists), sales-order printing (no print view), a US state layer, per-employee official fields, ZATCA QR/XML, and official-values research for the seventeen rules-only countries. |
| **Only the Owner chooses the Studio's country** | ✅ BUILT (18/09/2026) | The owner's requirement. The country decides which official values exist and what prints on every legal document, so a settings editor may fill the values and may not change the country: the settings PUT answers `owner-only`, and the row says so. A country change is written into the official-values history. |
| **Official values start empty; `legalInfo` is left as it is** | ✅ DECIDED (18/09/2026) | The owner: the Owner fills the values from scratch. Nothing is copied from the free-form Legal information rows, because a guessed mapping would put a number under the wrong label on a tax invoice. `legalInfo` stays for anything a country file does not name, and slices B and C decide how a value in both is not printed twice. |
| **The US package is federal only** | ✅ DECIDED (18/09/2026) | The owner's answer to the Phase 1 question. EIN, state of formation, the state's entity number and the registered agent. The state layer (sales tax permits, state withholding) comes later. |
| **Seats are for one person: session limits, a sharing flag, a lock PIN and paired tills** | CHANGED TO: **no session limit and no trusted-device cap** — the owner, 19/09/2026: "remove limitations for users". The 2-computers-and-1-phone limit, the step asking which session to end, and the cap of three trusted devices are removed; the console's sharing flag keeps its new-devices signal (the forced-sign-out signal only the limit produced went with it) and still shows and filters sessions per person. Everything else below stands. | ✅ BUILT (18/09/2026) — nine commits, `7ea7d42d` to `6f2e1e8f`, then passkeys (`@simplewebauthn`) and a per-part security reset in `/super` (two-factor, PIN, passkeys; the person is emailed); the desktop app left aside on the owner's instruction; model tests (`session-security-model`), tsc, strict tsc and the lint budget green; not opened in the sandbox (another session's dev server held the folder). `docs/functionality/sessions-and-devices.md` is the file. Not built: two-factor and passkeys in the desktop app. | The owner found the loophole: a five-seat company can hand one login to twenty people. The terms already forbid shared logins (`legalTerms.ts`) and nothing enforced it. The owner's reasons are all three: seat revenue, the customer's security and the audit trail. **Found on the way:** a session row records no device, sessions and trusted devices are both capped at 10 by silently dropping the oldest, the device list does not mark the device you are on (so a person removing one cannot tell which is theirs), and the device type is guessed from the user agent alone, which files Android tablets as Phones and iPads as Computers. **Decided:** (1) each person may hold **2 Computer sessions and 1 Phone session**; a **Portable Device shares the Phone slot**. The type comes from the user agent plus touch points and screen size reported at sign-in, and the TOTAL is what holds, since all of it is self-reported. A sign-in over the limit asks which session to end (oldest by default), and the ended one says why. (2) Trusted devices are capped to match, with no silent eviction, and the list marks "This device". (3) `/super` → Users gets a **sharing flag** raised by the signals (forced sign-outs in 7 days, new devices in 30, active sessions), a filter on it and on active sessions, and a **Send email** button with a fixed warning quoting the terms, written to the audit log. **Suspend stays a manual click**. (4) **The user sets their own idle timeout** in `/account` (bounded, no "never"), and idle **locks** rather than signing out. (5) **A lock button beside the profile, for the whole account** (every studio and every tab), unlocked by a personal PIN; enforced on the server (a locked session is refused on every call and gets no live data); 5 wrong PINs end the session and forget the device. The same PIN is asked again before signing an approval. (6) **Tills are paired to a device**: each till has a code, a manager pairs one device per till, POS opens **only** on a paired device (no exceptions, managers included), cashiers switch by PIN, and a paired till uses no person's device slot. **The plan limits tills: 1 until the plan says otherwise.** (7) Authenticator-app two-factor for users (reusing `superMfa.ts`), then passkeys. **Not taken:** a cheap view-only seat and a shared-terminal mode (the owner: no). **Build order:** "This device" marker · device types · session limit · the `/super` flag · device cap · lock/idle/PIN · paired tills · PIN at signing · two-factor. |
| **Marketing email from `/super` must meet anti-spam law from its first send** | ⬜ TO BUILD WITH IT (18/09/2026) | The owner's instruction, for when marketing is built in `/super`. **Today nompany sends no marketing email at all**, only system email (login codes, invitations, security notices, the seat-sharing warning), and CAN-SPAM does not cover those, so nothing needs to change now. The first promotional send changes that. **Required on every marketing email:** a working unsubscribe link, honoured within 10 business days and never behind a sign-in; nompany's physical postal address; a truthful sender and subject line; and a clear statement that it is an advertisement. Under GDPR and the Gulf's data laws, the recipient must also have **opted in** first, not just been given a way out. **So the build needs:** a per-person marketing-consent flag, recorded with when and how it was given, and separate from the system emails nobody can turn off; a suppression list that every send checks; and the unsubscribe link and address built into the template, not typed in each time. The US fine is up to about $53,000 per email. |
| **Server errors and scheduled jobs are reported to Sentry** | ✅ BUILT (19/09/2026), off until `SENTRY_DSN` is set | The owner chose Sentry after comparing it with PostHog, Google Analytics, ip-api and Fingerprint (19/09/2026). **Server only:** thrown errors (`withRequest` and Next's `onRequestError`), every `log.error` line as a message, and a check-in for each of the five crons (missed, failed or over 15 minutes). Request bodies, headers, cookies, query strings and the user are removed before sending (invariant 18). **No browser SDK**, for the bundle budget. Not built: browser errors, source-map upload, alert rules. `docs/functionality/error-tracking.md` is the file. **To switch it on:** create the Sentry project and set `SENTRY_DSN` in Vercel — the owner's step, not a session's. |
| **Fingerprint checks sign-in and sign-up** | ✅ BUILT (19/09/2026), server checks off until `FINGERPRINT_SECRET_API_KEY` is set | The owner asked for it for three uses: a new device asks for the code, signup abuse, and rate limits per device. **Built:** the agent runs on the sign-in and sign-up pages only, loaded on demand (its own chunk, on no page's first load); the server asks Fingerprint's EU Server API about the event and never believes a visitor id from the browser. A trusted device is **bound** to the browser trusted in; bad bots are refused (403 `automated`); failed passwords are counted per device (10 in 15 min) and new accounts per device (3 a day). **Fingerprint off or down changes nothing**; a withheld or forged event asks a bound device for the code. **Not built:** a proxy integration (so ad-blocker users of a bound device get the code every time), a privacy-policy line naming Fingerprint (legal copy for the owner to approve), any use of VPN/incognito/suspect score. `docs/functionality/device-intel.md` is the file. |
| **Marketing is a department: campaigns first, sixteen subsections to follow** | ✅ BUILT (19/09/2026), slice 1 of the owner's Marketing plan — campaigns-model, restructure, marketing, studio-setup, roles, departments, widget-sections, numbering, notices and access tests green; tsc, strict tsc, lint (0 errors); opened in the sandbox in English and Arabic (a campaign made through the form with its tracked link, a sub-campaign, a run, a completion, a clone, and every refusal) | The owner's *Marketing Section — ERP Requirements & Implementation Plan* (19/09/2026): seventeen subsections behind one dashboard, six phases, about 40 weeks. **Decided with the owner:** this slice is the section and Campaigns, the parent of everything else; **leads stay Sales tickets at the Lead stage and Marketing reads them**. Built: the eighteenth department, its dashboard (running, starting this week, needing attention, budget in open campaigns counted once, expected revenue and leads, by status, by channel) and the campaign register (objective, channels, dates, owner, budget, targets, one level of sub-campaigns with the budget they take, the UTM set and a tracked link, a Draft ⇄ Planned → Running ⇄ Paused → Completed/Cancelled ladder, *Run again*). Rights `marketing.dashboard`, `marketing.campaigns` (catalogue +5); the winner-of-work shape holds both; **existing roles gain nothing** (no predecessor right), an Admin grants them. On for every trade by default and asked about at creation. `docs/functionality/marketing.md` lists the sixteen not built. **The owner's answers, 19/09/2026:** nompany sends no marketing email or SMS for now, and studios use their own tools for it; nompany does not resell messaging or ad credits (both are rows below, for later). There is no website builder. **Still open:** what a landing page and a form need without one, and what AI would be for (both explained to the owner, not yet answered). **Next slice, by the plan's Phase 1:** a Sales ticket names its campaign (so actual leads, customers and won value show per campaign), then forms and the UTM capture, then Audiences & Consent. |
| **Linking a studio's own social account (Meta, LinkedIn, X, TikTok)** | ⬜ LATER, IF EVER (23/09/2026) | Asked by the owner while reviewing what Journeys and the four channel subsections would need. **The owner's answer, in their own words: "it does take nompany from recording marketing work to performing it" — keep it as a future plan.** That is the whole decision and it is not a technical one: the same line separates every built Marketing subsection (which RECORD what a studio does) from the four that are not. **What the investigation found, written down so nobody re-derives it.** The hard part is already solved and in the tree: `platform/auth/calendarProviders.ts` + `calendarOAuth.ts` + `calendarConnections.ts` are a complete third-party account-linking stack — one config row per provider (authorize/token/revoke URLs, scope, and the per-provider spelling of "give me a refresh token"), its OWN redirect path so a data grant is never mistaken for a sign-in, tokens AES-256-GCM encrypted at rest under a purpose subkey of `NOMPANY_DATA_KEY`, and a public type built by NAMING its safe fields rather than deleting two from a spread, because a spread that forgets a field added later leaks a token silently into every log. Adding a provider is adding a row. **THE ONE STRUCTURAL DIFFERENCE, and it is not a port:** every calendar connection is keyed by `userId`, because a calendar is a PERSON'S. A studio's Instagram page is the COMPANY'S — keyed by user it silently belongs to whoever clicked Connect and leaves with them. A social link needs a studio-scoped store, a right deciding who may connect and disconnect, and an answer to what happens to scheduled posts when that person goes. **And the cost is approval, not code:** Meta needs Business Verification and App Review per permission, LinkedIn needs Marketing Developer Platform partner approval for Company Pages, X's free tier will not carry a studio's posting, TikTok and YouTube both need review. Each is also a PER-TENANT compliance burden if studios connect their own pages under nompany's app. **The cheaper middle step, if this is ever revisited:** Social Media as a PLANNING and RECORD subsection — what is scheduled, on which channel, with the asset attached (the library exists) and the tracked link — published by hand. No app review at all, and the OAuth seam stays available. |
| **Email and SMS delivery from inside nompany** | ⬜ LATER, IF NEEDED (19/09/2026) | The owner: nompany has no delivery layer, and for now **each company runs its own email and SMS tools**; campaigns record the work and the tracked link, and nothing is sent from nompany. Build it only when studios ask for it. If it is built, the Marketing plan recommends **buying** delivery (an SES, SendGrid or Mailgun class provider behind one provider interface) rather than running mail servers, with per-studio sending domains, throttling on complaints and double opt-in for imported lists, because one studio's bad list would otherwise hurt every studio's delivery. It also brings the anti-spam duties the `/super` marketing email row lists: consent, unsubscribe, suppression, a postal address. |
| **nompany reselling messaging and ad credits** | ⬜ LATER, MAYBE (19/09/2026) | The owner: **not now**. A studio that sends WhatsApp, SMS or ads does it through its own accounts with the providers. Reselling credits (nompany buys volume and bills studios per message) may come later; it would need a credit balance per studio, a price list in `/super` and billing through the subscription. |
| **A Sales manager assigns each lead to a sales executive** | ✅ BUILT (19/09/2026) — leads-model, campaigns-model, restructure, roles, notices, access, pipeline and sales-analytics tests green; tsc, strict tsc, lint budget; opened in the sandbox (a campaign with a 24-hour deadline sent a lead; it reached the Sales queue unassigned with its campaign and deadline; assigned through the queue; a second assign, clearing the assignee and re-tagging the campaign were all refused; the assignee's comment stopped the clock; the campaign and the dashboard counted the lead). `docs/functionality/leads.md` is the file. **The owner's answers to the proposals, 19/09/2026:** the deadline is set **per campaign**; Marketing and Sales each have a right for managing and assigning (`marketing.campaigns.assign` chooses a campaign's owner, `crmSales.tickets.assign` hands out leads); whoever has full Sales access sees unassigned leads (the assign right; existing roles that edit Sales' settings catch up to it); the executive is notified; a campaign that sends a lead is its original source and never changes; existing tickets keep their raiser as assignee. What was built is below, as it was proposed. | **Why:** a Sales ticket has no assignment today. `assignedToCollaboratorId` means "who raised it", is set from the session and cannot change (`modules/sales/sales.ts`), so a lead from a form (raised by nobody) or typed in by Marketing has nobody working it and nothing to hand over. **Decided by the owner, 19/09/2026:** (1) **a manager assigns by hand**; no rotation and no rules; (2) **two fields**: "raised by" stays as it is and never changes, and a new "assigned to" is set and changed by the manager, every change kept in the ticket's history; (3) **a deadline**: a lead nobody has assigned, or an assigned one nobody has touched, alerts the manager; (4) **an executive cannot reject a lead**; only the manager can reassign it. **Proposed, not yet confirmed:** the deadline is 24 hours by default and set per studio in Sales settings; assigning is its own right (an extra on `crmSales.tickets`, so being able to edit a ticket does not mean being able to hand it to someone); unassigned leads are listed only to holders of that right; the executive is notified on assignment; a ticket names its campaign (a picker, "Which campaign brought them?"), which gives each campaign its actual leads, won deals and won value. Existing tickets keep their raiser as their assignee, so nothing changes for them. |
| **Marketing Forms: studio-built forms on the questionnaire builder** | ✅ BUILT (19/09/2026), the owner approved the shape the same day — forms-model, leads-model, campaigns-model, restructure, sealing, roles, studio-setup, widget-sections, marketing and access tests green; tsc, strict tsc, lint budget; opened in the sandbox (an Enquiry form created from its template, named to a campaign and opened; answered through the public page and the API: missing consent, a bad email and a bot refused, two answers stored encrypted and read back, each raising an unassigned Sales lead with the campaign and its 12-hour deadline; the Responses summary, CSV and lead links; the QR code; the form embedded in an iframe; delete refused once answered; an Arabic survey right-to-left with a branching rule that blocks only when it reveals its question). `docs/functionality/forms.md` is the file. **Built as proposed below**, with fifteen question types (file upload, signature, picture choice, ranking and matrix left out); the public address is `/f/<slug>/<code>`. | The owner asked for a form tool like Google Forms for the Marketing team, and for nothing fixed. **The questionnaire builder already has** pages, 26 question types (contact, phone, email, consent, NPS, rating, matrix, file upload, signature…), branching with author warnings, preview, and a response summary, table and CSV (`docs/functionality/questionnaires.md`). **What it lacks for studios:** it is nompany's own and built in `/super`; only signed-in users answer, once each. **The proposal:** a Marketing subsection **Forms**, laid out like Google Forms: templates (Blank, Enquiry, Event registration, Customer feedback) and recent forms; an editor with **Questions / Responses / Settings** tabs reusing the builder and the answering screen; **Settings** holds the campaign, "create a Sales lead" with which questions fill name, phone and email, who is notified, accepting responses, a closing date and the confirmation message; **public answering** without sign-in, many replies per form, spam protection, and a required consent tickbox when contact details are collected; **sharing** by link, QR code and one general embed code for any website; the studio's logo and colour. A submission set to create leads becomes a Sales ticket at Lead, tagged with its campaign, unassigned, and waits for the manager (row above). **One tool serves three subsections of the plan:** Forms, Events (registration) and Research & Feedback (surveys, NPS). |
| **Marketing Budget & spend: a bill or an expense names its campaign, and Marketing reads what it cost** | ✅ BUILT (21/09/2026) — marketing-spend-model (new), marketing, campaigns, restructure, roles, access and the rest of the model chain green; tsc, strict tsc, lint (0 errors); **opened in the sandbox on 22/09/2026** in English and Arabic (two campaigns budgeted, a bill and an expense filed against them from Finance's own forms, 125% read as over with the amount, an unbudgeted campaign read as "no budget set" rather than as zeros). **The sandbox found a real defect the tests did not:** deleting a campaign took the studio's spent tile from 8,800 to 6,300 while its bill sat untouched in Payables — the screen reported a studio as having spent less BY TIDYING ITS REGISTER. The total carries `unattributed` now, as `projectCosting` already did with `uncoded`. A second fix from the same session: the four tiles go four-across only from `xl` rather than the house `lg`, because `StatTile` CLIPS rather than ellipses and 12,000.00 rendered as "12,000.0" — **a shared defect of `StatTile` that every other tile row still has**. `docs/functionality/marketing-budget.md` is the file. | The second part of the owner's plan to ship, chosen 21/09/2026 as the next slice: a campaign had carried a budget and three targets since 19/09 with **nothing ever measured against them**. **Built:** `campaignId` on a bill and on an expense — the mirror of a bill's `costCodeId` and an invoice's `milestoneId`, unvalidated at the write and attributed by the reader, so a since-deleted campaign's costs land in `unattributed` and are counted rather than lost; a campaign picker on both Finance forms, offering open campaigns only through the shared `referencePickers`; a `marketing-budget` section owning **no collection**; the pure `spend.ts` (a sub-campaign spends its parent's money, a cancelled campaign's spend still counts, null rather than zero on every unknown, over at 100% and nearly-spent at 80%); and the join Marketing existed to make — cost per lead, cost per won deal and return on spend, against the leads the campaigns already count. A foreign bill converts at **the rate it was booked at**, today's table second, and is left out and counted when neither can answer. Right `marketing.budget`, **view alone** (setting a budget is editing the campaign; filing a cost is editing the bill); the winner-of-work shape holds it and whoever views campaigns catches up. **Nothing is spent from Marketing** — one ledger, Finance's. **Not built:** committed spend (a purchase order cannot name a campaign yet), spend over time, and alerts to anybody. |
| **Marketing Audiences & consent: the ledger of who may be contacted, and on what evidence** | ✅ BUILT (22/09/2026) — consent-model (new), sealing, marketing, restructure, roles and access tests green; tsc, strict tsc, lint (0 errors); opened in the sandbox, where a public form answered WITH the consent box ticked wrote two ledger rows (email lowercased, phone reduced to digits) carrying the question's own wording, and the same form answered without it was refused. `docs/functionality/audiences.md` is the file. **The sandbox found two defects the tests did not:** a date in the FUTURE was silently replaced with today and reported as saved, and a BACKDATED withdrawal left the address allowed while the screen said only "added" — somebody would have believed they had stopped the contact. The first is refused now (`consent-future`); the second is kept (it happened, and a ledger that refuses inconvenient history is not evidence) with the resulting state named back: "this address is now: Allowed", plus a line saying the entry changed nothing. A third, found by the dev overlay: the row fragments had no React key. | Forms have refused to open without a required consent question since 19/09, so every studio already puts the tick in front of the public — and **nothing recorded the answer**; it sat in one sealed response row at a time. **Built:** section `marketing-audiences` owning `marketingConsents`, APPEND-ONLY (no update, no delete, in the service and in the route: a consent somebody could edit proves nothing); one address is one subject, normalised, so a withdrawal against `Ali@Firm.com` stops a send to `ali@firm.com`; the latest decision wins and **"never asked" is its own answer** rather than a no; the evidence is the consent question's wording COPIED at the moment it was answered; `value` sealed at rest (invariant 18). Two channels, `email` and `phone` (covering calls, SMS and WhatsApp), because one tick cannot mean four permissions. Written from a form tick with the studio's own authority whether or not a lead is raised, and by hand for whoever writes in or telephones. Right `marketing.audiences` view/edit; whoever reads form answers catches up to view. **Nothing checks the ledger yet** — `mayContact` is pure and uncalled, because nompany sends nothing (the owner, 19/09/2026). **Not built:** lists and segments, per-channel consent, a public preference centre, data-subject requests (erasure against an append-only ledger needs a decision), bounces. |
| **Lead scoring: the queue is ordered by how strong a lead is, and every score says why** | ✅ BUILT (22/09/2026) — lead-scoring-model (new), leads-model, sales-analytics, restructure and access tests green; tsc, strict tsc, lint (0 errors); opened in the sandbox (a form-sourced lead with a company, both contact routes and a description scored 60 warm against 25 cold for thin ones; late leads still sort first; the chip opens to its reasons in both languages). `docs/functionality/leads.md` is the file. **Found by the dev overlay:** the reasons panel used `<ul>` inside the row's `<p>`, which is invalid HTML and broke hydration — rebuilt from spans. | The queue was ordered by LATENESS, which answers "what have I neglected" and not "what is worth doing first". **Built:** `modules/sales/scoring.ts`, pure, seven declared factors summing to 100 — reachable 25 (half for one route), a company rather than a person 15, budget stated 15, has bought before 15 (half for an open deal), said what they want 10 (half for an industry alone), told us about the job 10, came from a campaign 10 — with hot at 70 and warm at 40. **Every score returns its factors, including the ones that earned nothing**, so the chip can list what is *Not known* and "cold" becomes something to act on. **Quality, not urgency:** lateness still sorts first, because a deadline the studio set itself outranks how good a lead looks. Scored only AT the Lead stage; past it the salesperson's own `probability` is the better number. **No permission key** — a score is a property of a ticket and answers to `crmSales.tickets.view`. **It predicts nothing:** no model, no training, just facts the studio holds weighted by readable rules. **Not built:** studio-set weights, a stored score history, decay with age, and anything learned from closed deals. |
| **Lead scoring counts what happened after the lead arrived** | ✅ BUILT (22/09/2026), the first of the four steps the owner ordered that day | Somebody who answered three forms and somebody who filled one in and vanished scored the same. **Built:** an eighth factor, `engaged` (15), counted from the consent ledger — one row per form answer per address, so no new field and one read of a small collection. Once earns nothing (that answer IS the lead), twice half, three times the whole. Only PUBLIC rows count: a consent recorded by hand is the studio’s own act, and counting it would let a studio raise a lead’s score by filing paperwork about it; an address’s email and phone rows are taken at the higher of the two rather than summed, or one answer would read as two visits. **The idea that made it honest:** a factor the studio CANNOT answer leaves the total rather than scoring nought — a studio with Audiences off is scored out of 85, its bands keep meaning what they mean, and the chip does not list the factor as missing. Weights rebalanced (reachable 20, company 12, wants 8, told 8, campaign 7) and every score moved with them, which costs nothing because none is stored. **Verified in the sandbox:** one address answered the same form three times and its leads went 47 → 55 → 62 while a one-off stayed at 47. **Next:** decay with age. |
| **A lead fades as it sits, and says what the age cost it** | ✅ BUILT (22/09/2026), the second of the four scoring steps | A three-month-old lead scored what it did on arrival, so one that had gone cold still looked hot. **Built:** a fortnight of grace, then a straight line to HALF by ninety days and no further. **The floor is the point** — a lead does not become worthless by ageing, it becomes less likely, and decaying to nothing would empty the queue of exactly the enquiries nobody got round to. **The clock runs from the most recent thing that happened**, not from the raise date: another form answer from the same address (the ledger carries the date) or somebody acting on it (`firstActionAt`) makes it live again. The chip says what the age cost, because a score lower than its own reasons add up to would otherwise be an unexplained deduction; `raw` travels beside `score`. A lead with no readable date does not fade — a missing date is not evidence of age. **Verified:** the arithmetic by test at 14, 30, 90 and 365 days and for both resets; live in the sandbox the clock-reset is visible (an address that answered again reads 0 days where its neighbours read 2), but a fresh sandbox holds no lead old enough to fade. **Next:** studio-set weights, when a studio asks for them. |
| **The Marketing dashboard is built LAST, not next** | ⏸ DEFERRED BY THE OWNER (22/09/2026) | Offered as the next slice on the grounds that marketing-sourced revenue, ROI and cost per lead became computable the day Budget & spend shipped. The owner: delay it until there are proper insights from the system and everything is in hand, then build a better one. **The reasoning is sound and is recorded so nobody re-offers it:** a dashboard built now would be designed around the four subsections that happen to exist (campaigns, forms, spend, consent) rather than the seventeen the department will have, and a dashboard is the one screen that should be designed from what the whole system knows rather than grown a widget at a time. Until then the dashboard keeps SAYING its figures are a plan, which is true and visible rather than quietly stale. |
| **Marketing Planning & calendar: what runs when, and what collides** | ✅ BUILT (22/09/2026), the calendar half of the plan's subsection | The register answers “what is on” and never “what is on AT ONCE”: four email campaigns overlapping in one week look reasonable on four rows and unreasonable in one inbox. **Built:** section `marketing-planning`, owning NOTHING — every bar is a campaign. Whole weeks from a Monday, twelve by default and starting a week BEFORE today (a calendar opening on the current Monday hides the campaign that started last Thursday and is still running). A bar is coloured by status rather than channel; one that began earlier is cut rather than dropped, one with no end runs to the edge and SAYS so rather than being given an invented date, a one-day campaign still has width, and a cancelled one is absent. **Campaigns with no dates are named underneath rather than omitted** — a studio with nine undated campaigns has a planning problem the calendar would otherwise hide. Above the chart: what starts, what ends, and **more than one at once per channel**, which is a warning and not an error because two at once is sometimes the plan. Right `marketing.planning`, view alone (moving a campaign in time is editing that campaign); whoever views campaigns catches up. **Verified in the sandbox** in English and Arabic with three dated campaigns: bars in the right weeks, the crowding warning firing on two overlapping email campaigns, the undated ones listed, and the Arabic ruler mirroring. **A test that was wrong, not the code:** a one-day campaign both starts and ends in the same week and belongs in both lists. **Not built:** plans by period (briefs shipped the same day, the row below). |
| **Events are on the marketing calendar, as markers rather than bars** | ✅ BUILT (22/09/2026) — marketing-calendar-model extended (14 new assertions), restructure and the marketing model chain green; tsc and lint clean on my own tree. | Two things built the same day did not know about each other: the calendar's own "not built" said *"Nothing but campaigns is on it — events and content deadlines are all invisible here"*, so a marketing calendar showed the campaigns running in a week and not the webinar on the fifteenth. **A MARKER, NOT A BAR, and that is the answer the file was waiting for.** A campaign OCCUPIES weeks — it is a span, and drawing it as one is what makes overlap visible — while an event is a POINT. A day-wide bar is a sliver nobody can read; a week-wide one claims a week of the calendar for an hour of somebody's time. **The marker and the ruler cannot disagree about which week a day is in:** both do the same Monday arithmetic, and the test asserts every marker's week is one of the ruler's own columns — drawing an event under the wrong column is the only way this join can be wrong and still look right. **Events are read only for a holder of `marketing.events.view`**, and a reader without it is not charged the round trip either (the customer-360 rule); the screen says whether they were read at all, so an empty week is never mistaken for a quiet one. **An event does not enter the crowding warning** — that counts campaigns competing for one CHANNEL's attention, and a webinar in a room competes with none of them. **MY OWN TEST CAUGHT A ROBUSTNESS GAP BEFORE IT SHIPPED:** `markers(null)` threw `events is not iterable`, while every other function in that file coerces whatever the store hands back — the same shape as the `t.dependencies is not iterable` crash that white-screened the whole planner. Coerced now, and asserted. **Not built:** forms with closing dates and content deadlines are still invisible on it. |
| **Marketing partners, PR and influencers: a partner owns a link tag, and what they brought is COUNTED** | ✅ BUILT (22/09/2026), the ninth of the plan's seventeen subsections — partners-model (new, 30 assertions), restructure, section-routes, access, roles, widget-sections, studio-setup and the marketing model chain green; tsc, strict tsc, lint (0 errors); **exercised end to end against the sandbox's REAL arrival data** left by the UTM slice: before any partner existed the screen named `newsletter` (2) and `facebook` (1) as tags arriving that nobody held; creating one with source `NEWSLETTER` folded it to `newsletter` and immediately read **2 arrivals · 1 lead · 0% won**; the tag left the unclaimed list; a second partner claiming it was refused twice (exactly, and in another case); and deleting the partner was refused `partner-brought`. Opened in the studio. | **Why this is a register and not an address book.** A partner owns the `utm_source` on the links they publish; a form reads that tag back when somebody arrives (built this morning); the reply carries the Sales ticket it became. So the chain runs their newsletter → an arrival → a lead → a won deal and its value, END TO END, and **the figures are counted rather than typed in** — there is deliberately no field for "leads they sent us", because a number a person maintains beside one the system can count is a second number free to disagree with it. **Built:** section `marketing-partners` owning `marketingPartners`, right `marketing.partners` (catalogue +4). **Two partners may not hold one tag**, refused at the WRITE rather than resolved at the read: every arrival under it would count for both and the studio would see its own numbers doubled with nothing saying why, and there is no honest way to split a submission between two claimants afterwards. Matched case-insensitively, because a URL is typed by hand and frequently by somebody who does not work here. **NULL RATHER THAN A ROW OF NOUGHTS, twice:** a partner with no tag shows no figures at all and the screen says why — "nobody has given them a link" and "their links brought nobody" are different facts and only one is the partner's doing — and a win rate is null with no leads rather than 0%, which would read as leads that all failed. **The tags nobody claims are NAMED**, the counterpart of the unmatched campaign tag on a form's reply, and frequently the most interesting row on the screen: the partner nobody realised they had. **The fee is a note, not an amount** — what a partner costs is a Finance bill naming the campaign, like every cost in Marketing. **A partner that brought anybody is kept**; ending the arrangement is what the state is for. **It opens figures derived from form replies and never a reply**, a name or an address: those are sealed and stay with `marketing.forms.view`. **AND A FINDING THAT STOPPED A SLICE BEING BUILT:** Marketing's *Promotions & Loyalty* was next in line, and **promotions are ALREADY BUILT** — Point of Sale's engine carries conditions, benefits, tiers, coupons and a channel covering online as well as the counter. Marketing must not grow a second one. What that subsection would add is LOYALTY, which is unbuilt and sits in the POS area another session is actively editing, so it was not started. **Not built:** nothing is sent to a partner (no brief, link pack or report — they cannot see their own figures), no commission or payout, no split by campaign (the tag joins, so a partner on three campaigns is one set of figures), only FORM arrivals are counted, and nothing but this screen reads the register. |
| **A target is measured against what happened, on the records that own the targets** | ✅ BUILT (22/09/2026) — attainment-model (new, 30 assertions), campaigns-model, marketing-plans, marketing-model and restructure green; tsc, strict tsc, lint (0 errors); **verified against live sandbox data and both screens**: CMP-0001 reads "Expected leads: 2 of 20 (10%)", a campaign with no target draws nothing at all rather than 0%, and the Q4 plan reads "Leads expected: 0 of 120 (0%)". | The third "declared, written, never read" defect found today, after the UTM tags and the scale questions. **A campaign** has carried `expectedLeads`, `expectedCustomers` and `expectedRevenue` since Marketing shipped: the register printed the targets on one line and what the campaign actually brought in on the NEXT line, and nothing ever compared them — `expectedCustomers` was not even summed. **A plan** has carried `expectedLeads` and `expectedRevenue` since that morning and they were read by NOTHING AT ALL. **Built:** `attainment.ts` (pure) — target against actual, with a share, whether it was met and how far short. **NULL RATHER THAN ZERO on both undefined states**, and neither is pedantry: a target nobody set shown as 0% reports a studio as having missed something it never asked for, and a target of NOUGHT divides into `Infinity%` on a card. A shortfall floors at nought, because beating a target is already in the share and "minus three leads short" is not a sentence. **A sub-campaign's leads are ADDED, not contained** — the opposite of its budget, and deliberately: a Sales ticket names exactly one campaign, so adding members' leads adds each ticket once, while a budget is an allowance handed down. That asymmetry is why `planAttainment` and `planRollup` are separate functions. **NOT the dashboard**, which the owner deferred until the department is in hand: attainment belongs where the person who set the target can see it. **No permission key** — it reads what the register and the plans screen already open. **Not built:** no trend, no attainment by channel or source, and nothing alerts anybody to a target being missed. |
| **Marketing content and brand assets, on a revision chain lifted out of Tendering rather than copied** | ✅ BUILT (22/09/2026), the eighth of the plan's seventeen subsections — assets-model (new, 30 assertions), tender-documents, events-model, restructure, section-routes, access, roles and the marketing model chain green; tsc, strict tsc, lint (0 errors). | A campaign gained a BRIEF earlier the same day — who it is for, what it says, what it offers — and there was nowhere at all to put what was MADE from it, so the artwork lived in somebody's Drive and "which logo was on the autumn adverts" had no answer inside the product that had the adverts in it. **Built in two halves. FIRST the extraction:** Tendering has kept a replaced bid document since the tender pack shipped, and assets need the same three rules for the same reason, so the chain moved to `lib/revisions` and `modules/tendering/documents` DELEGATES to it — keeping its own refusal token (`other-tender`, not `other-parent`) so no string anybody reads moved, and `tests/assets-model` asserts the sharing **by identity** rather than by behaviour, because two implementations that agree today are precisely what the extraction prevents. That file's own guard said "imports nothing"; it is narrower rather than weaker now — exactly one import, by name, and the module it names is held to the original rule, so a `@/platform/db` import there still fails. **THEN the library:** section `marketing-content` owning `marketingAssets`, right `marketing.content` (catalogue +4); a kind that says what an asset is FOR rather than what program made it; an OPTIONAL campaign, because a studio's logo belongs to the studio and a library that could not hold the brand itself would send people back to the Drive; and an asset whose campaign was deleted falling into that same group rather than vanishing, since deleting a campaign must not delete artwork somebody paid a designer for. **The file is `/api/media`, never bytes here and never a Blob URL**, and its id is checked against this studio — the route refuses a non-member, but an id is a string in a body, and without the check one studio could file another's id and the record would claim a file the reader resolves to nothing. **The file is set once, on create:** swapping the bytes under a name is exactly what the version chain exists to prevent. Deleting takes the blob first and the record second (invariant 11). **The campaign is NOT a chain boundary**, the one place this differs from a tender's pack: artwork genuinely moves between campaigns and the new file is honestly the next version of the old one. **A SECOND COERCION-BEATS-THE-RULE BUG, the same shape as Events' capacity an hour earlier:** `assetFields` mapped any unrecognised kind straight to `"other"`, so `assetProblem`'s KIND COERCION refusal could never fire — a body carrying `kind: "psd"` got past the kind check and failed on the file instead. Found by opening the API, not by the model test, which asserts the rule the service was stepping around. `cleanKind` keeps an unknown kind now so the rule refuses it, and an omitted one still becomes the catch-all. **AND THE UPLOAD PATH IS UNVERIFIED:** `BLOB_READ_WRITE_TOKEN` is EMPTY in `.env.local`, so `/api/media` answers 500 to every upload locally — an environment limit affecting every upload in the product (form file questions, the tender pack, HR documents, the quality workflow), not this feature. Everything that does not need a stored file was exercised against a running studio; creating a real asset, the chain end to end and deleting a file rest on the model test alone. **Not built:** no preview, no folders, tags or search, no approval, no record of where an asset was used, and nothing but this screen reads the library. |
| **Marketing events and webinars: an event counts its sign-ups from a form, and owns who actually came** | ✅ BUILT (22/09/2026), the seventh of the plan's seventeen subsections — events-model (new, 38 assertions), crud (a new lifecycle entry), restructure, section-routes, access, roles, widget-sections, studio-setup and the marketing model chain green; tsc, strict tsc, lint (0 errors); **exercised end to end against the sandbox and opened in English and Arabic**: an event over its capacity read "7 registered · 4 over capacity", the door listed all seven registrants and marking two gave "2 of 7 came · 29% turnout · 5 did not come", a foreign id was refused `not-registered`, and delete was refused once anybody had been marked. **The sandbox found a defect the model test could not:** `capacity: 0` was ACCEPTED (201) because the service's coercion turned any non-positive number into `null` — "no limit" — before `eventProblem`'s refusal could ever see it, so a studio typing 0 had it silently read as the opposite. The rule was correct and unreachable; the coercion now keeps a number that is not allowed so the rule can refuse it, and `tests/crud.mjs` covers the route lifecycle that the model test cannot reach. `docs/functionality/events.md` is the file. | **Built:** section `marketing-events` owning `marketingEvents`, right `marketing.events` (catalogue +4). An event is a name, a kind (a room or a wire), a START STAMP rather than a date (an event starts at six) with an optional end, a place, a capacity, an owner, and links to a campaign and a registration form. **IT STORES NO REGISTRATIONS** — the form tool has done registration since it shipped, so an event NAMES a form and its sign-ups are that form's replies, counted where Forms keeps them; a second list of the same people would be free to disagree with the first. **What it DOES own is attendance**, because nothing else in the product knows who walked in, and the gap between registering and turning up is the only number an event exists to produce. Stored as response IDS, never names: the names are sealed form answers (invariant 18) and a copy would put a stranger's details in a second, unsealed place. **The stored list may go stale and the count never does** — an id for a since-deleted reply drops out at the READ, the containment where `projectBilling` puts it, because no write-time check covers a later deletion. **Capacity informs and never refuses:** blank is no limit (not a limit of nought, or an open webinar reads as full), nought is refused outright, oversubscription is shown rather than clamped, and the public form is never turned off by it — a page that started rejecting people because a figure moved in Marketing would be a silent outage. **The registrants are NOT this right's to give:** `marketing.events.view` opens the counts, and who signed up is read only for a holder of `marketing.forms.view`, with the screen SAYING so rather than showing an empty list. Whoever works the campaigns catches up verb for verb. **Not built:** nothing is sent (no invitation, reminder or calendar invite), no waiting list, no phone check-in, no walk-in who is not already a form reply, no per-event cost, no recurrence, and events are not drawn on the marketing calendar. |
| **A scale answer means something: NPS, and an average for a rating** | ✅ BUILT (22/09/2026) — scales-model (new, 40 assertions), questionnaire-model, forms-model, restructure green; tsc, strict tsc, lint (0 errors); **verified against the sandbox's live Arabic feedback form**, where the NPS question went from six frequency-sorted bars and no score to **NPS −67 · 1 promoter · 0 passives · 5 detractors** with eleven bars in scale order, and the rating to "5 out of 5". `docs/functionality/forms.md` is the file. | `nps`, `rating` and `opinion-scale` have been question types since the questionnaire was built, and `summariseResponses` counted them exactly as it counts a multiple-choice — one bar per distinct answer, **sorted by how often each came up**. An NPS question drew "9" above "2", **nothing computed a Net Promoter Score anywhere in the product**, and a five-point rating had no average: the type declared its meaning and the summary threw it away, the same class of defect as a field written and never read. **Built:** `lib/questionnaireScales.ts` (pure) — the standard bands (0–6, 7–8, 9–10), score as %promoters less %detractors, the mean for a rating, and a chart in SCALE order with every point present, because a tally list that omits an unpicked value draws a four-bar chart of a five-point scale. **Null rather than nought on both figures**, and here it is not pedantry: an NPS of 0 is a genuine result (promoters and detractors cancelled), so a defaulted zero would report an unanswered form as performing averagely, and an average of 0 on a 1–5 scale is OUTSIDE the scale. An out-of-bounds answer is counted into the band it borders rather than dropped. **In `lib`, not in Marketing**, so nompany's own `/super` questionnaires get the same arithmetic. **A regression this introduced and caught:** the `/super` summary sized its bars from `tallies[0].count`, correct only while tallies were commonest-first — on a scale the first bar is the lowest point and is frequently nought, which would have drawn every bar full; it asks the whole list for its maximum now, as the studio's screen already did. **No permission key. Not built:** no trend over time, no score by campaign or source, and no follow-up driven by the band. |
| **Marketing reads its own tracked links back: a form knows which campaign brought the person** | ✅ BUILT (22/09/2026) — arrival-model (new, 38 assertions), forms-model, restructure, section-routes and the marketing model chain green; tsc, strict tsc, lint (0 errors); **proven end to end against the sandbox**: the Enquiry form's settings name *Website push*, a submission tagged `utm_campaign=autumn-launch` raised a Sales ticket carrying *Autumn Launch* while an untagged one carried *Website push*, a `autum-sale-typo` tag was credited to nobody and reported as unmatched, and a `https://www.google.com/search?q=x` referrer was stored as `www.google.com` with its query dropped. `docs/functionality/forms.md` is the file. **THE CODE LANDED INSIDE COMMIT `8772f7e3`**, another session's POS commit, which took it off the index while it was staged — the branch-per-session hazard CLAUDE.md warns about, arriving from the other direction. | Every campaign has published tagged links since Marketing shipped and **nothing had ever read one back**: the visitor arrived at `/f/<slug>/<code>?utm_campaign=…`, the form threw the query away, and the lead was credited to whatever was typed into that form's settings. One form serving three campaigns credited all three to one — and lead scoring's campaign factor, leads-per-campaign on the register and cost-per-lead in Budget & spend all rested on that hand-typed field. **Built:** `arrival.ts` (pure); the five tags plus the referring HOST, reduced at the boundary because a full referrer is somebody else's page address with their own query on it; the tag matched against the studio's own campaigns the same three ways `taggedLink` may have written it — **through the same `utmSlug` that writes it**, so the two halves cannot drift — and an AMBIGUOUS tag credits nobody, because two campaigns may carry one tag and crediting the first is a silent coin toss. The LINK beats the settings; the settings stay the fallback for a bookmark or a printed QR code. **An unmatched tag is reported rather than swallowed**, which is what forced `attributedBy` onto the reply: a credited campaign beside a tag that named nothing is otherwise indistinguishable from an ordinary untagged fallback, so a studio whose live advert points at a deleted campaign would never be told. **No permission key** — the arrival is the form's own content. **No backfill:** an older reply carries no arrival and shows a dash rather than an invented campaign. **Not built:** the tags are read on the FORM alone, so there is no first-touch or multi-touch attribution, and nothing but the Responses tab reads the arrival. |
| **Marketing plans by period: the envelope above the campaigns, and the campaigns nobody filed under it** | ✅ BUILT (22/09/2026), completing Planning & Calendar — marketing-plans (new, 40 assertions), section-routes, restructure, access, roles, campaigns and the marketing model chain green; tsc, strict tsc, lint (0 errors); exercised against the sandbox end to end (a quarter taken from the 14th of November stored as 01/10 → 31/12; a campaign filed and its 10,000 budget and 6,300 of real Finance spend rolled up; a nameless plan, a backwards range and a `planId` naming nothing all refused; delete refused while a campaign named it) and opened in English and Arabic. **It found a defect that was NOT this feature's:** `marketing/audiences`, `marketing/budget` and `marketing/calendar` had never been added to `SECTION_BY_ROUTE`, so a studio that switched any of those parts off could still read them through the API while the sidebar showed nothing — `tests/section-routes-model` had been red about exactly that since each shipped and was being read as a known failure. All four rows are in now. `docs/functionality/marketing-calendar.md` holds the feature. | A campaign was the largest unit Marketing had: every budget, target and date sat on one, so "what are we doing this quarter and what is it allowed to cost" could only be answered by adding the register up by hand. **Built:** `marketingPlans` under `marketing-planning`, the section that until now owned nothing; a name, a whole calendar period resolved on the server from any day inside it (so two plans called Q1 cannot disagree about when Q1 starts), what the period is FOR in words, an envelope and two targets. **A campaign NAMES its plan; nothing is inferred from dates** — a campaign running from 20 March to 10 April falls inside two quarters, so a window-claiming plan would count its budget twice and neither total would be true. The price of an explicit link is the campaign somebody forgot to file, paid back by `unplanned`: every plan NAMES the live campaigns running inside its dates that belong to no plan, with one click to file each. Money: a sub-campaign counted inside its parent and never twice, spend read through Budget & spend's own reader rather than a second copy of the currency handling, and **null rather than zero on every figure derived from a budget nobody set**. `marketing.planning` gained create/edit/delete (catalogue +3) with a SECOND catch-up entry rather than an edit of the view one, whose id is already stamped on every role it has answered for; the winner-of-work shape holds it at full. **Not built:** no ladder and no approval (a plan is written, not signed off), no rollover, no period-against-period comparison, the targets are stored and nothing measures against them, and a plan is not drawn as a band on the calendar. |
| **Marketing campaign briefs: four questions, so a campaign can be judged on what it set out to do** | ✅ BUILT (22/09/2026), the brief half of Planning & Calendar — campaigns-model (a brief block), restructure and the marketing model chain green; tsc, strict tsc, lint (0 errors); opened in the sandbox, where a brief written through the API came back with its author and the one question left, and the register drew *No brief yet* on the three that had none. | A campaign carried a budget, three targets, a ladder and a tracked link, and **nothing saying what it was FOR** — so "did it work" was answered afterwards against whichever figure was to hand. **Built:** `brief` on the campaign (audience, message, offer, success, with who last wrote it and when), answering to `marketing.campaigns.edit` and minting **no right** — the brief IS the campaign's content, and a second right over the same act would be free to disagree with the first. **Four questions rather than one box**, because a box gets a paragraph and a question gets an answer; the fourth ("what would make it a success?") is the one nobody writes unprompted and the one the judgement later needs. **An unstarted brief is not a brief with four gaps:** `briefGaps` is empty until some question is answered, so a register does not open reading as behind on every campaign — *No brief yet* and *1 question unanswered* are different states and say so. Whitespace is not an answer; a long answer is CUT to its limit rather than refused, so a pasted page loses its tail instead of losing the whole edit. `docs/functionality/marketing.md` holds it. **Not built:** assets and files on the brief (a campaign has no attachments yet), and no approval — a brief is written, not signed. |
| **Customer insights: the owner's 3 + 3 + 1 buying-pattern analysis, in CRM & Sales** | ✅ BUILT (19/09/2026) — insights-model and every model test, restructure, tsc, strict tsc, lint (0 errors); opened in the sandbox in English and Arabic (twenty invoices across six customers read as loyal, returning `H·0·L`, slipping, new, lapsed and dormant, with movement since the month before; two customers sent to Sales as unassigned leads naming their pattern, then skipped on a second send; the CSV in Arabic with its byte-order mark; quarters; the locked teaser on the Basic tier). `docs/functionality/customer-insights.md` is the file. | The owner's Excel system for a sales department: month-end invoice exports stacked into one sheet, pivots of invoice count and value per customer per month, and each customer's last seven months read as 3 + 3 + the current month, each block zero/low/medium/high, every shape named, plus a scatter of sales by person, team and channel. **Decided with the owner:** in CRM & Sales for now; levels relative to the studio, with the amounts shown; the default pattern names (loyal, growing, steady, fading, slipping, returning, lapsed, new, stopped, dormant); periods of a month, quarter, half-year or year; its own right. **Built:** sub-section `crm-sales-insights` (owns nothing); a sale is an issued invoice less its credit notes, or a till receipt naming a customer, in the studio's currency; won deals and till sales feed the scatter; two advanced-tier widgets; **send to Sales** raises unassigned leads through `raiseLead` with why each was sent, skips customers with a deal open, and notifies the assigners once; CSV. Right `crmSales.insights` (view, extras export and act; catalogue +3); winner-of-work holds all three, a department head views; roles that read the Sales dashboard, or assign leads, catch up. **A tier whose widgets were chosen by hand in `/super` must tick the two new ones.** |
| **Tasks is replaced by Approvals, and no trace of Tasks is left** | ✅ BUILT AND LIVE (19/09/2026). Converted and removed on production the same day | The owner. **Approvals replaces the Tasks board, its settings, its right (`tasks.board`), its section (`tasks`, `tasks-settings`) and every reference to it — code, tests, docs and this file included.** Nothing is typed by hand: an approval exists only because a record asked for one. **Raised by a Request approval button on the record** (not automatically, until everything is tested), and filed with where it came from (section, subsection, record link) and who requested it. **The status lives on the approval and the record reads it** — carried, never copied. **Every approval is assigned; nothing is self-approved.** Each approval type is set up in Approvals settings as ordered steps, answered one after the other; each step names specific studio members, with a checkbox for whether ALL of them must approve or ANY ONE. No department heads, no roles — named people. An approval has an overall status and each approver has their own. **Migration:** every existing task — typed and hand-written — is read, turned into an approval carrying its overall and per-person statuses, then **all tasks are deleted**. Statuses carry over as Done → Approved, Open and In progress → Pending, **Blocked → Rejected**. A hand-written task becomes an approval whose requester is the person who wrote it and whose approver is its assignee, filed as carried over from the old board — the only hand-written approvals there will ever be (invariant 17: export first, delete by explicit id list, re-scan, two confirmations at run time). **Scope:** the 84-item inventory taken 19/09/2026 (chat) — the six engines already signing (bill, bid, requisition, stock adjustment, till return, payroll), the ones with their own approve step (leave, expense claim, change order, timesheet, document revision, held payment) and the seven task types come first; the owner will name which of the rest follow. **Held aside, to check type by type while building:** requester-cannot-approve and its Admin exceptions (invariant 7), the signer PIN, the amount limits, and the ten gaps the inventory found (a quotation approved by editing its status, a journal reversal that ignores a closed period, a certifier nobody compares with the writer, and the rest). **Step 1, the engine, is built and pure** (`modules/approvals/{schema,registry,model,fromTasks}.ts`, `tests/approvals-model.mjs`, no store, no screen): the steps are FROZEN onto an approval when it is requested, as a bill freezes its plan, so changing the settings never changes who was asked; the requester is taken off any step others share and a request whose only approver is the requester is refused; a no from anybody at the open step rejects the whole approval and must give a reason; an answer is final; one open request per record and type. Being named on the open step IS the authority to answer — no permission is asked. Storage, the section and the rights land with step 2, where who may open the page is decided. **Step 2, the page and its settings, is built** (`modules/approvals/approvals.ts`, two routes, `StudioApprovals.js`, a mark beside the logo next to Tasks until Tasks leaves): everybody opens it and sees what is waiting on them and what they asked for; `approvals.overview.view` adds every approval and `approvals.settings` sets the steps — the owner's and Admins', given to others in Access, no library role born with either; the owner and Admins may answer their own request (the owner, 19/09/2026). **Step 3, first slice: the two approvals the old board already raised from a button** — Send for Approval (a ticket's quotation, or an internal one from Technical) files a Quotation approval, and Submit PO files a Client PO approval whose approval issues the project number. Every reader — Sales' ticket, Technical's list and lock, the RFQ gate, openProject, the Inventory sheet's PO search, Main's "Awaiting you" — asks `modules/approvals/reads.ts`. **A quotation can no longer be approved by editing its status** (`needs-approval`); ones stored Approved before stay so. Approvals' `source`, `note` and `attachment` are sealed (invariant 18). **THE ROLLOUT CONSEQUENCE: a quotation approved on the old board reads as NOT approved until the conversion (step 6) runs, so steps 3–6 must reach production together, with the conversion run straight after the deploy.** Waiting on the owner: which record each of the other five types (Material PO, Delivery, Delivery return, ID update, Permit request) is requested from. **Step 4, the conversion tooling, is built and has NOT been run anywhere** — `scripts/migrate/tasks-to-approvals.mjs` (dry-run by default, additive, idempotent: each approval keeps its task's id, so a re-run converts nothing twice and no field on an approval names Tasks) and `scripts/migrate/remove-tasks.mjs` (invariant 17 in full: refuses while `tasks` is still in SECTION_DEFS, refuses a studio with an unconverted task, exports everything to a named file before the first delete, deletes task rows by explicit id, re-scans, then removes the two section rows through cascade.ts). **The order on the day: deploy steps 3–5, run the conversion, run the removal (two confirmations), then a final commit deletes both scripts and `fromTasks.ts`.** **Step 5, the board removed from the code, is built:** the module, its routes, screen, copy, rights (`tasks.board`, `tasks.settings`), sections, notification type, deal member type and relation edges, Nova's task tools (Nova reads a person's approvals instead and answers none), and every mention in comments and docs — Main's count, "Awaiting you", recent feed and Nova's bubble read approvals, and an approval shows in the feed only to somebody it concerns. The lint ceiling came down 100 → 95. `remove-tasks.mjs` now also detaches every task from its deal, strips `tasks.*` from roles and personal overrides, drops the board's notifications and the `task` stage from saved flow templates, and clears the Nova switches — all exported first. Main's rollup counters for the old section are pruned by the nightly reconcile on their own. **Rehearsed end to end in the sandbox, 19/09/2026:** an old board seeded (5 tasks — a part-signed quotation, a signed client PO, two by hand, one routed to nobody — plus a role right, a notification and a saved template), converted (5 written, a second run 0), read on the Approvals page and answered, then removed with the owner's two confirmations (export first; 5 tasks, 22 roles, 1 notification, 1 template, 2 section rows) and read back independently: nothing left, the 5 approvals intact, and a planting read of the section list did not bring the board back. **Live, 19/09/2026, once production was on `9f620850`:** the conversion wrote 1 approval (a signed quotation in `nompany`, Approved), and a second run wrote 0. After the owner confirmed twice, the removal exported first, then deleted 1 task, 12 section rows and the `tasks.*` rights of 136 roles across 6 studios; there were no personal overrides, notifications, saved templates or Nova switches to clear. Each studio re-scanned empty, and a separate read-back found no tasks sections, rights, notices, templates or Nova switches anywhere, and the one approval intact. **Then `fromTasks.ts` and both scripts were deleted.** The `carried` approval type stays, because converted approvals use it. |
| **Every approval moves onto the Approvals page; the request stays where it is made** | ✅ BUILT (19/09/2026): all twelve types answer on the Approvals page, and the old chain engine is gone | The owner, 19/09/2026: *"on the same place the approval is requested like returns from till move all the approvals onto Approvals"*. **Twelve types:** till return, bill, bid, purchase requisition, stock adjustment, payroll run, expense claim, change order, timesheet, document revision, held-payment release, leave. **The owner's five answers:** (1) a step may start at an amount (`from`), so "a bill has step 1 for everyone and step 2 from 50,000"; (2) each type's `approve`/`approveHigh` rights are removed as it moves, and its steps are seeded with the people who hold them today; (3) nothing stops on deploy — until a studio saves a type, its default steps are today's right holders plus the owner and Admins, worked out on read (`defaultSetting`), never stored; (4) every yes asks the signer's PIN, every type; (5) requests already waiting on the old engines are carried: each gets its approval the first time its list is read, in the requester's name and keeping the signatures it already had — it adds rows and deletes nothing, so no export or second confirmation was needed (the Tasks board, which did delete, had both). **The owner and Admins may answer their own request on every type** (bids, requisitions, claims and releases did not allow it before). **Built:** the engine (thresholds and a frozen amount on the approval, the under-every-limit pass, `distinctSigners` for reviewer ≠ approver, the PIN on the approvals route, `effects.ts` — the record moves when its approval is decided, with the studio's authority through `moduleContext(...).asApprover`, is asked first whether it can, and records a failure on the approval with Try again); **the till return** (asking is the request; the answer, restock and refund happen from the Approvals page; `pos.returns.approve` and its catch-up are gone; a return waiting from before is given its approval on the first read of Returns); **the stock adjustment** (recording one over the lowest limit is the request; approving writes the movement; `inventory.stock.approve`/`.approveHigh` are gone and the chain editor no longer offers it; one waiting from before gets its approval on the first read of the queue, **carrying the signatures it already had**); **the bill** (a Request approval button on a received bill in Payables — the owner's on-click rule; approving makes it Approved, which payment waits on; a bill waiting on its approval refuses a change to its amount; a no changes nothing on the bill; `finance.payables.approve`/`.approveHigh` are gone and the chain editor no longer offers bills; a bill part-signed from before gets its approval with its signatures carried, and a received one nobody had signed waits for the button); **the bid** (Request approval beside the fully priced bill; submitting reads the approval, and only while it is for the value the bid has now — a repriced bill needs asking again; nothing is written on the tender; `tendering.tenders.approve`/`.approveHigh` are gone and the chain editor no longer offers bids; a bid part-signed from before is carried). **the requisition** (submitting is asking; a request with an unestimated line walks every step, closing gap 9; `procurement.requisitions.approve`/`.approveHigh` are gone). **The old chain engine is gone** with the last of its types: its walker, validator and Studio settings editor, Finance's chain plumbing; the seeds and stored chains stay, read only as a moved type's default steps. **Payroll** (Request approval on a draft run; `hr.payroll.approve` is gone; nobody is told when a run is merely prepared); **the expense claim** (submitting is asking; the Admin may now answer their own, the owner's uniform rule; `finance.claims.approve` and its catch-up are gone); **the change order** (submitting is asking, the value delta counted absolute; `crmSales.contracts.approve` is gone); **the timesheet** (submitting is asking; it had no right of its own, so the default is whoever holds `projects.list.edit`; no screen submits one yet); **the document revision** (sending for review files a two-step approval — review, then approval — that nobody answers both steps of, the owner included; the review yes moves the revision to `approval` through a new `stepped` hook; a document's own named signers override the settings; `engineeringDocs.register.review`/`.approve` are gone); **the held-payment release** (Request release with a reason; the approver writes the release and may not then pay; `finance.payables.release` is gone); **leave** (requesting is asking; a yes Approves it and a no Declines it; the requester may still withdraw it; a manager filing somebody else's leave still decides it on the spot; Main's own leave bubble is gone, the Approvals bubble says it; `hr.vacations.approve` is gone). **All twelve types are on the Approvals page.** **Still open, from the owner:** which of the single-right acts (RFQ award, subcontract certification, period close, asset disposal, offboarding) become approvals. |
| **A studio screen paints with its data: the page answers the screen's own GET in its render** | ✅ BUILT (23/09/2026) — every department screen but those listed below; tsc, strict tsc, lint budget and restructure green; opened in the sandbox (Main, Sales, Finance payables, HR, Projects, People, Studio settings, Maintenance orders, Marketing planning, Quality & HSE, Contracts, an engine register, Inventory, the FS schedule), and none fired its first read | The owner, 23/09/2026: a section click showed two skeletons ("double loading") where `/super` shows one. Cause: the page rendered an empty screen, and the screen then fetched its own route, which resolved the studio again from scratch. `route()` now also returns `firstPayload(slug, path, params)`: the same handler and context builder, run inside the page's `withRequest` so the re-resolution comes from the request cache. The page hands the body down as `initial`; `useReload(load, initial)` (or the same identity skip in a screen's own effect) skips the first fetch. Refused, over the 48 KiB ceiling, or thrown → no `initial`, and the screen fetches as before. **This replaces the design's view function per route** (`2026-09-06-server-rendered-first-payload-design.md`): one mechanism, no hand-extracted copies. **Six GETs moved onto `route()` to take part** — main, main/engagements, collaborators, roles, settings, technical — each with the new `keys: false`, so they still refuse API keys as they did; they now answer a locked session 423 (was 401) and refuse a till session. **Still fetching on mount, deliberately:** Point of Sale's dashboard, sales and shifts (their range comes from the browser's clock and time zone, which the server cannot reproduce); Finance's tax screen (two routes, no primary); the full-screen editors and viewers (till, document editor and print, planner and plan print, form builder, the two Live views, the manual); every secondary read (dialog options, later tabs, "load more"). |
| **Promotions are a Point of Sale subsection, and the till keeps its own discount** | ✅ BUILT (22/09/2026) — five slices: registration, the pure engine, the till and the receipt, the admin screens, activation-by-approval and package gating. Verified: restructure, section-routes, access, roles, numbering, widget-sections, approvals, pos-model, pos-return-model, pos-promotions-model, lint budget | The owner's brief: an offer the till prices a basket with — tiers, benefits, coupons, schedules, caps — kept apart from the CASHIER's discount, which is unchanged. Decided with the owner on the day: **scope is a TILL, not a branch** (the product has no branch, and multi-site is separate studios); **schedules read the STUDIO's timezone**, a Studio setting rather than a POS one, on the owner's rule that anything several sections can use is not a section's setting; **conditions name items, `itemType` and the vendor** until items have a category axis — which was BUILT the same day, as a register rather than the taxonomy axis the plan below described, adding `item_category` beside the older token rather than re-pointing it; **customer tags become a renameable register** with ids, so a rename re-tags nobody; **consent is Marketing's existing ledger**, not a second field; **gating is sold in `/super`** and studio-dependent settings stay in the section that owns them; and the **cashier's cap is measured after an offer has priced the line** — otherwise an automatic offer above the cap refuses the whole sale, which is a defect the cap check has carried since it was written. Two things are parked on the owner's instruction: **emitting a discount as an e-invoice allowance** (beside ZATCA in `einvoicing.md`) and **the SQL-level constraints the brief asked for**, which this store cannot express — rows are JSON payloads in one shared table, a module never writes SQL, and there is no transaction or row lock, so uniqueness and dates are enforced in code and a coupon's limits by compare-and-set on the coupon's own row. **What shipped beyond the brief's shape, and why.** **The engine is pure and there is exactly one of it** — the till screen, `createSale` and the editor's live preview all call `evaluate`, so a price shown, a price charged and a price previewed cannot be three answers. The server re-prices at the sale with the usage counters a till is not given and answers `promotions-changed` with both figures rather than ringing the sale up at a total nobody has seen. **A coupon is claimed BEFORE the sale is written and released if the write never lands** — there is no transaction spanning the two, so the order is chosen: a claim nobody used is a number somebody can put back, a single-use coupon spent twice is money given away twice. **Usage is counted from the redemption rows**, one per offer per sale, never from a counter on the offer: a counter beside the rows it counts parts company with them the first time a sale is written and its bump is not. **Activation is the signable act**, not the write: a draft prices no basket, and editing a live offer is already impossible, so nobody can activate something harmless and rewrite it. What an offer is WORTH is the most it could cost (per-sale cap × times it may be used); cap neither and it is null, which walks every step. **The timezone became a STUDIO setting** (`shared/timezone.ts`, Studio settings beside the currency) on the owner's instruction, and the POS module's private copy of "which day is it" was deleted the same day rather than left beside it. **Gating is two package switches in `/super`** — Promotions, and separately coupons/tiers/ schedules — and both ABSENT mean on, so no existing package lost anything. It bites at the WRITE and never at the read: a plan change must not alter what somebody at a counter is charged. `docs/functionality/promotions.md`. |
| **nompany's packages and tiers are priced by region, Steam's way** | ✅ BUILT (23/09/2026), NOT OPENED IN A BROWSER — price-regions-model (46 assertions), marketing-model, restructure, tsc, strict tsc, eslint | `docs/functionality/regional-pricing.md`. The owner: price packages and tiers by the buyer's location like Steam, and the payment method must be from the same country. A **region** is a set of countries sharing one currency with prices FIXED per package, band and tier (`shared/priceRegions`, stored at `g:priceRegions`, edited at `/super/regions`). An unfixed price is a SUGGESTION — base × today's rate × the region's price level, rounded to two significant figures. The public pricing page shows the visitor's region only (**the currency picker is gone**, the owner's choice) and prices are **before tax**, with the rate from catalogue settings (`taxPercent`, default 16) named under the cards. **Nine regions are planted on first read** (the owner chose a Steam-like set): Jordan, Saudi Arabia, UAE, Qatar, Kuwait, MENA (USD), Europe (EUR), United Kingdom, Rest of world (default) — every level 100 and **no price fixed**, so until the owner fixes prices the live page shows rounded conversions. The payment-method rule (`billingRegionFor`, `ibanCountry`, `regionChangeProblem`, 90 days) is written and tested and **called by nothing yet: there is no checkout**. **Tiers are on the pricing page** since the owner said a tier's cost is per month, on top of the package. Also fixed on the way: tiers showed "SAR" on a USD base list, and the price formatter dropped every currency's decimals but SAR's. |
| **A studio's subscription to nompany is dates, and its status is worked out from them** | ✅ BUILT (23/09/2026), NOT OPENED IN A BROWSER; **CHANGED TO (24/09/2026): the rules in the row "Only Standard has a free period…" below** — its trial-for-every-studio and 3-month grace are replaced — subscription-model, restructure, marketing-model, plan-limits, tsc, strict tsc, lint budget | `docs/functionality/subscriptions.md`. One document per studio (`g:subscription:<id>`) holding the subscription and its append-only history; `paidUntil` is the one date, and trial / active / complimentary / past due / read-only / cancelled is computed from it in Amman time, never stored. Only a payment moves it forward and a reversal back; an event id is applied once; paying after going read-only restarts from the payment day. **The owner's numbers:** trial 3 months, grace 3 months (catalogue settings); **existing studios complimentary** (planted on first read, no script); **the Free package ENDS** — read-only the day its Duration (3 months) is up, with no grace, unless the studio picks a paid package (the owner, 23/09/2026); the trial length is the package's own Duration, the figure its pricing card states; **extra seats blocked** — which also closed compound packages having no member limit at all. Console: status and paid-until columns in `/super → Studios`, a Subscription panel in the studio dialog (record a transfer, reverse, seats, period, extend trial, complimentary, cancel/resume, history). **Read-only is computed and NOT enforced yet**; no reminders, invoices, checkout or owner Billing page. |
| **A studio its owner deleted is deleted thirty days later — and the job reports until switched on** | ✅ BUILT (24/09/2026), SWITCHED OFF — studio-deletion-model, restructure, tsc, eslint | `docs/functionality/studio-ownership.md`. The owner believed this was built: the settings screen counted down 30 days and **nothing acted on the date** — no job read `deletionRequestedAt` and `cascadeDeleteStudio` had no caller. Now `shared/studioDeletion` owns the 30 days (the screen and the job read it), and `cron/studio-deletions` (daily 02:45 UTC) deletes due studios through the cascade, five per run, re-reading each right before. **Invariant 17: it only REPORTS until `STUDIO_DELETIONS=on`** — the second confirmation. It also deletes the studio's **uploaded files** (outside its prefix, so the cascade never reached them) and sets its **billing record to expire ten years out**. **Terms 1.3 and Privacy 1.1 match** (24/09/2026): unpaid fees get three months' grace then read-only, never deletion; the free Standard package lasts three months; deletion 30 days after the owner asks, files included; billing records 10 years after. The terms' `effective` date was NOT moved: whether §17's 30-day notice applies is the owner's call. |
| **Only Standard has a free period; a paid package is paid before it applies; an unpaid studio closes at 20 days, shuts down at 90 and is deleted at a year** | 🟡 STEP 1 BUILT (24/09/2026), NOT OPENED IN A BROWSER — subscription-model, studio-deletion-model, restructure, tsc — `docs/functionality/subscriptions.md`: the rules and the ladder in `shared/subscription`, ONE gate (`gateRequest`/`subscriptionRefusal`) on the route wrapper's session and key paths and on every raw studio write route (a test fails a new one that skips it), uploads gated, the studio's banner and shut-down screen, the console panel on the ladder with payment-for-a-package; Terms 1.4 state the timings, **effective 24/10/2026** (§17's thirty days). **STEP 2 BUILT (24/09/2026)**: seats — the paid seats else the package's ceiling (`shared/seats`); compound packages had NO limit and now take their largest band; the seat is counted inside `addCollaborator`'s own write so two approvals cannot race past it, and a losing approval reopens to pending; `/super → Studios` shows used / limit, red when over. (There is only one door besides creation — join-request approval — measured 24/09/2026.) **STEP 3 BUILT (24/09/2026)**: the upgrade button in the studio header (owner, Standard) and on /account's owned studios opens one dialog; the choice is priced in the owner's region with tax (`shared/upgradeQuote`, a year = 12 × the per-month yearly figure) and stored as `studio.upgradeRequest`, a REQUEST nompany answers by recording the transfer — the console pre-fills it, a payment now carries its billing period, and recording it moves the studio and clears the request. Open in a closed or shut-down studio. **STEP 6 BUILT (24/09/2026)**: download everything (`/export`, owner only, every record read through `readCol`, secrets left out), the warning emails (`cron/subscription-notices`, 30/7/1 days before shut-down and deletion, in the studio's language, recorded only once sent), and deletion at day 365 inside `cron/studio-deletions` behind its OWN switch `UNPAID_DELETIONS` (report until set) and only after the one-day warning went out. **TRACKING (24/09/2026)**: a Billing watch card on /super → Studios (`nextStep`, soonest first, with warnings sent and upgrade asked), each sent warning as a `warning-sent` history line, /super notifications for a warning that failed and a studio held from deletion as not-warned, and a sandbox-only clock (`lib/sandbox`, 404 elsewhere) that rehearses the ladder — which found the layout's date formatter had lost its backslashes, so every banner and the shut-down screen printed no date; fixed. **BANDS (24/09/2026, the owner)**: a studio on a compound package is on a band — `categoryId` on the studio, the seat limit is the band's size (seats still override; no band keeps the largest, nothing guessed), a Band dropdown in /super, the band carried by upgrade requests and recorded payments, shown beside the package; packages not on sale (the old Premium, three live studios) are marked in the dropdown. Not yet: checkout (step 5). | The owner, 24/09/2026, replacing the 23/09 trial and grace. **Standard** (free, 1–4 users) is the only package with the 3 months, counted **per studio**, and taking them is optional — the owner may pay any time. **Paid packages start at 5 users; there is no trial on one**; a package applies to the one studio it was paid for, once paid. **A studio that has paid can never return to Standard's free 3 months**; its owner may still open new Standard studios while they have free-studio room, each with its own 3 months. **Repeated Standard studios are not restricted.** **A seat is the owner plus everyone who joined**; every package's limit is enforced at every door (today only join-request approval checks it), and `/super → Studios` shows used / limit. **Prices follow the visitor's location and are locked per invoice.** **Upgrade** sits in the studio header for Standard studios and on `/account`'s owned studios. **The unpaid ladder:** invoice at the START of the period (day 0) → unpaid at day 20: **closed**, everything viewable, nothing created or changed → day 90: **shut down**, members locked out, the owner sees only pay and download-everything → warning emails 30, 7 and 1 days before → day 365: **deleted** through the existing `cron/studio-deletions` (report mode, 10-year billing retention). Paying any time before deletion restores at once. A Standard studio at the end of its 3 months takes the same ladder, closed that day. **The questionnaire merges into studio creation** (field, country, city, ERPs), except questions about the PERSON (why they use nompany), which stay separate. **Before the ladder runs on real customers:** a full download-everything export, and the Terms stating the timings. Still open: downgrade at renewal or at once; how a balance is used on the next invoice. |
| **Customers pay nompany by bank transfer and say so; nompany confirms, invoices and refunds from /super** | ✅ BUILT (26/09/2026) AND OPENED IN THE SANDBOX: Payments saved (IBAN stored as `enc:v2`), a claim held a closed studio at full access, confirmed into NMP-2026-00001, refund request answered with NMP-CN-2026-00001 taking the month back, Arabic Billing checked; it found invoices dated in UTC, fixed (4aac3651) — subscription-model, restructure, tsc, tsc strict, eslint — `docs/functionality/subscriptions.md` → "Paying by bank transfer". The owner: online payment waits until customers arrive, so **step 5 (checkout) is on hold and bank transfer is the way to pay**; it stays a way to pay afterwards. **/super → Payments** is the permanent home of every payment method (bank transfer on, card listed as coming later), nompany's invoice details, the email claims go to, and the claim hold. Bank details are **encrypted at rest** (`fieldCrypto`, a subkey of NOMPANY_DATA_KEY), and so are a claim's payer, bank reference and note, a refund's reason and the buyer on every invoice. The owner sees the details and **"I've sent the transfer"** in the upgrade dialog once a package is requested and on **/account → Billing** (the owner's answer to "where?": the account page, because it is the customer's business with nompany and a shut-down studio cannot be opened). A claim rings the bell and emails nompany; it **holds the ladder for 48 hours by default** (the owner: transfers take 1–24 hours, sometimes more), never blocks the studio, and **keeps a studio from unpaid deletion until it is answered**. Confirming records the payment, moves the studio onto the plan and **issues nompany's tax invoice** (16%, the customer's currency, NMP-YYYY-NNNNN); rejecting emails the owner the reason. **Refunds**: the owner asks against an invoice, or nompany records one itself; either way a `refunded` event (paid time taken back only when asked), a **credit note** against the invoice, and an email. Studio banner and shut-down "Pay" now open Billing, not the contact page. **Not built:** JoFotara submission of these invoices, receipt uploads, card payments. |
| **The home page shows the real product: captured screens, a tour, the record's journey, a language slider and live charts** | ✅ BUILT AND PUSHED (26/09/2026) — tsc, strict tsc, lint (91, unchanged), marketing-model, restructure; opened in both languages, both themes and at phone width through the capture driver; `next build` green and `/[locale]` inside its baseline (the studio, account and login routes were already over theirs from other work, which this commit does not touch) | The owner, 26/09/2026: *"real screenshots, real components, interactive visuals and animations"* — the page was text and one drawn dashboard with invented figures. **The screens are real captures** (`scripts/screenshots.mjs`): a demo company, *Qimam Contracting & Trading* (invented, Jordan, JOD — the owner's choice), built in the SANDBOX through the product's own services by `scripts/lib/demo-seed.mjs` — 8 people, 15 clients, 26 deals at every stage, 5 projects with plans, costs and milestones sized from their real value, stock received on purchase orders, requisitions signed by a second person, invoices and supplier bills — then photographed by `scripts/lib/edge-capture.mjs`, which drives the Edge already on the machine over the DevTools protocol (Playwright stays out), at 1440×900 2× in both languages and themes: 52 WebP files in `public/screens/`, every one captioned on the site as sample data. **The page** (`components/landing/showcase/`, copy in `shared/marketing/tour.ts`): the hero is the real pipeline at full width; a tour of the owner's three featured departments (Sales & pipeline, Projects & Gantt, Inventory & procurement) that plays until touched; *One record, start to finish*; an English/Arabic slider (a real `role="slider"`); the product's own chart kit live with a period switch. Below-fold sections load in their own chunks and still server-render. `DashboardAssembly` (the drawing) is deleted. **Found on the way, not fixed here:** the planner ignores the studio's dark theme; CLAUDE.md says requisitions keep reviewer ≠ approver for everyone, but the code lets an owner or Admin approve their own. |
...
## E-invoicing adapters — the work list (25/09/2026, rewritten 26/09/2026)

**THE RULE, the owner's, 26/09/2026: nompany PREPARES, the company SUBMITS.** An adapter builds
each invoice's official file from the authority's published instructions. The company downloads
it, submits it through the authority's own channel, and records the answer here. **Nothing in the
product contacts a tax authority**: no API, no credentials, no certificates, no one-time
passwords. The ledger row "E-invoicing adapters, one per country" holds the decision and what was
removed; `docs/functionality/einvoicing.md` is the file. **The 16/09/2026 rule still stands:
build a country's adapter when a studio in that country needs one.** Re-check a country against
its tax authority's published instructions before starting it.

**1. Saudi Arabia (`zatca`): 🟡 PREPARES SIMPLIFIED INVOICES (26/09/2026).**
- ✅ **The file:** ZATCA's UBL in canonical form, tax per category on the total, the counter and
  hash chain, and the QR's five seller tags. It is unsigned, because the phase-2 stamp belongs to
  the company's own certified solution.
- ✅ **Once, before the rule:** on 25/09/2026, with the owner's go-ahead, signed documents built
  from this code passed ZATCA's public developer portal. That confirmed the canonical form, the
  hash and the QR time (which must carry no `Z`). Not to be repeated.
- ⬜ **Credit and debit notes.** Refunds are not prepared, so a studio over-declares output tax.
  The composer already takes both.
- ⬜ **Business (B2B) invoices:** the buyer's VAT number and national address on the client record.
- ⬜ **Zero-rated and exempt lines need a VATEX reason** on the line. Until then they are refused
  rather than written with a guessed code.
- ⬜ **The printed Saudi invoice:** check it carries everything ZATCA requires on paper, Arabic
  included.

**2. Jordan (`jofotara`): 🟡 PREPARES INVOICES (26/09/2026).** ISTD's UBL from the tax number
and two new official values: the JoFotara income source sequence and invoice code. The QR is
JoFotara's, pasted back when the studio records the answer. **Its submission to JoFotara was on
`main` from 22/09 and was removed 26/09/2026.**
- ⬜ **The first file a studio submits and records as accepted** is the only check this can have.
  A rejection recorded with ISTD's words is the next fix.
- ⬜ **Credit notes**, held by the owner until a first invoice is accepted (22/09/2026).

**3. Shared by every adapter ⬜:**
- a discount as a UBL `AllowanceCharge` (parked by the owner, 22/09/2026);
- a buyer's tax registration on the client record;
- **opening the Tax screen's Download and Record in `dev:sandbox`**, which has not been done
  since the 26/09/2026 rewrite.

**4. Countries whose file could be prepared ⬜**, roughly by mandate date: **Egypt** (ETA
e-receipt), **Kenya** (KRA eTIMS), **Croatia** (Fiscalization 2.0), **Peru** (boleta), **Oman**
(Fawtara, from 01/04/2027), **Spain** (VERI\*FACTU, from 2027), **Brazil** (NFC-e), **Colombia**
(POS ticket), **Mexico** (CFDI 4.0). Several of these require the authority's approval before the
receipt is handed over, and under the rule that is the company's own step. Each country's
research is in "Country document rules" below.

**5. Not something a prepared file can serve.** These need certified hardware, certified
software or a fiscal register: Germany (cloud TSE), Austria (RKSV), Portugal, France (NF525),
Chile, the Basque Country (TicketBAI), Italy (until 2027), Poland, Turkey (below the invoice
threshold). **No e-invoicing beyond a correct receipt:** UAE, Bahrain, Kuwait, Qatar.
...
### An item category axis — BUILT (22/09/2026), and not as the plan above described it

This section planned a SEVENTH TAXONOMY AXIS. It shipped as a REGISTER instead, and the
reason is the one `clientTags` records: a taxonomy value is stored BY NAME on every record
using it, so renaming one strands them — and a category is exactly the thing a shop renames
and re-arranges. Ids, nesting four levels deep through `shared/departments/tree`, a tab
under Master data. `docs/functionality/master-data.md`.

**WHAT READING THE CODE CHANGED ABOUT THE PLAN, and it is the part worth keeping.** The plan
called `itemType` "free text with a per-vendor list", which is true and much too mild. It is
the chosen SUPPLIER's product line: the item form fills that dropdown from the vendor's own
`itemTypes`, and picking one sets `deliveryWeeks` from the vendor's row. So it is scoped to
one supplier and load-bearing for procurement. Two suppliers selling helmets produce two
unrelated strings; an item with no vendor has no type at all.

That made "the promotions conditions read the axis instead" — this plan's closing line — the
wrong move twice over. **`itemType` is not repurposed** (it would break lead times), and
**`category_in_list` is not re-pointed at the register**: every offer already written with
that token would have been silently re-scoped from "this supplier's line" to "this category",
which are different sets of goods, and a name→id migration would have looked like a rename
while being a widening. `item_category` is a NEW condition token beside it. Nothing was
migrated because nothing changed meaning.

**An offer on a parent matches its children.** The line carries its whole ancestor path
(`pathIds`, walked once per request) because the engine is pure and cannot walk a register it
was never handed — so matching is an intersection rather than a tree walk in the hot loop.

**Not seeded; suggested.** The register starts empty and offers the distinct `itemType`
values already on the studio's own items, for a person to pick from. Importing them would
have been wrong rather than merely presumptuous: a supplier's line is frequently not a
category, and two spellings of one category arrive as two.

**Still open:** `itemImport` maps a spreadsheet's "Category" column onto `itemType` and has
not been re-pointed; no bulk re-filing exists, so categorising a large shelf is one visit per
item; and Reports, Inventory and Procurement do not read categories yet — only Promotions
does. A BRAND axis is the same shape and is not built.
