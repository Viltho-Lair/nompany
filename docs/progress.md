...
**WHAT IS STILL UNVERIFIED:** no screen in this section has been OPENED — the sandbox could not
start (`cloud-sql-proxy`), so every slice since meters is verified by pure tests, types and lint
alone — and the store-backed suite has not been run.
