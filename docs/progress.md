...
### An item category axis — a future plan (22/09/2026)

An item has `itemType`, free text with a per-vendor list, and no category or brand. The
studio-wide taxonomy (`administration/taxonomy.ts`) already opens six closed lists —
client industries, expense categories, payment methods, leave types, location kinds, permit
types — and items are not one of them. **Adding `itemCategories` as a seventh axis** would
give Inventory, promotions and reporting one vocabulary a studio can extend, instead of a
free-text field each reader groups differently. It is its own change: the reader has to be
added with the axis (invariant 16 one layer down), `itemImport` already maps a spreadsheet's
"Category" column onto `itemType`, and every existing item carries whatever was typed.
Promotions ship without it and match on item lists, `itemType` and the vendor instead.
