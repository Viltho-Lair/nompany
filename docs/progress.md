...
| **Promotions are a Point of Sale subsection, and the till keeps its own discount** | 🟡 SLICE 1 BUILT (22/09/2026) — restructure, section-routes, access, roles, numbering, widget-sections | The owner's brief: an offer the till prices a basket with — tiers, benefits, coupons, schedules, caps — kept apart from the CASHIER's discount, which is unchanged. Decided with the owner on the day: **scope is a TILL, not a branch** (the product has no branch, and multi-site is separate studios); **schedules read the STUDIO's timezone**, a Studio setting rather than a POS one, on the owner's rule that anything several sections can use is not a section's setting; **conditions name items, `itemType` and the vendor** until items have a category axis (below); **customer tags become a renameable register** with ids, so a rename re-tags nobody; **consent is Marketing's existing ledger**, not a second field; **gating is sold in `/super`** and studio-dependent settings stay in the section that owns them; and the **cashier's cap is measured after an offer has priced the line** — otherwise an automatic offer above the cap refuses the whole sale, which is a defect the cap check has carried since it was written. Two things are parked on the owner's instruction: **emitting a discount as an e-invoice allowance** (beside ZATCA in `einvoicing.md`) and **the SQL-level constraints the brief asked for**, which this store cannot express — rows are JSON payloads in one shared table, a module never writes SQL, and there is no transaction or row lock, so uniqueness and dates are enforced in code and a coupon's limits by compare-and-set on the coupon's own row. `docs/functionality/promotions.md`. |
