...
| **E-invoicing adapters, one per country** | 🟡 **JORDAN BUILT, NOT VERIFIED (22/09/2026)** — the owner chose to start with Jordan; every other country is still the 18/09/2026 framework-only decision. **BLOCKED ON CREDENTIALS, AND DELIBERATELY SO.** What is left needs a taxpayer's **sandbox Client-Id, Secret-Key and income source sequence** from the JoFotara portal (API Settings → create an application). Nothing has been submitted to ISTD, sandbox or otherwise, and a studio must enter its own credentials before anything is sent at all — that gate is what keeps an unproven adapter out of somebody's books. **CREDIT NOTES ARE HELD UNTIL THEN, on purpose** (22/09/2026): they would inherit every assumption the invoice makes — `currencyID="JO"`, element order, scheme attributes, a 107-page guide read structurally rather than line by line — so building them now means writing the same mistake twice and fixing it twice. The first ACCEPTED invoice is what validates those assumptions; credit notes are built on top of that, not beside it. Until they exist a Jordanian studio reports its sales and not its refunds, which over-declares output tax — so this is a real hole held open knowingly rather than an oversight. **Clearance gating is held for its own reason:** in a clearance country an invoice is not valid until accepted, but switching that on against an unverified adapter would stop a studio invoicing at all. It is a decision for after the sandbox. | The framework (`einvoicing.md`) is built and waits for adapters; each is one entry in `EINVOICE_ADAPTERS` keyed by the name the country's definition already declares. **Saudi Arabia (`zatca`)**: ZATCA phase 2 — signed UBL 2.1, invoice hash chain, cryptographic stamp, nine-tag TLV QR, clearance (B2B) and reporting within 24h (B2C), onboarding with a Fatoora-portal OTP; the heaviest. **Jordan (`jofotara`) IS BUILT (22/09/2026)**: `POST /core/invoices/` with `Client-Id` and `Secret-Key` headers and a body of `{"invoice": "<base64 UBL>"}`, answering `EINV_QR`, `EINV_NUM` and `EINV_INV_UUID`. Three files — a pure document composer, a UBL serialiser that knows no country, and the adapter that posts — plus a Studio settings panel for the studio's own credentials (sealed; the response says whether a secret is set, never what). **ISTD's own technical guide corrected a third-party tutorial in EIGHT ways**, each now a test: ProfileID required and first; `cbc:InvoiceTypeCode` always **388** with Jordan's own code in its `name` attribute, and the codes are five PAIRS rather than 011/012/013; `currencyID="JO"` rather than ISO 4217's `JOD`, in 283 consistent occurrences; a line uses `cac:TaxTotal` with a `cbc:RoundingAmount` and never `cac:ClassifiedTaxCategory`; tax ids carry `UN/ECE 5305` and `5153` scheme attributes; **a discount is `cac:AllowanceCharge` inside `cac:Price`** — which is where the parked discount mapping goes; `cbc:TaxCurrencyCode` beside the document currency; and `cac:SellerSupplierParty` carries the income source sequence. **The returned QR prints automatically** on the document (not a layout element: a fiscal QR is required by law rather than chosen by design, like the official values beside it), and **a daily cron retries transport failures only** — never an authority's rejection, which would be a studio hammering its tax office daily with a document a person must fix, and never an unsubmitted invoice, which waits on a person's decision. Each needs the authority's test account to verify against, and in a clearance country issuing an invoice should then wait on acceptance, which today it does not. Build one when a studio in that country needs it (the 16/09 rule). | The plan (18/09/2026: nine sub-sections, one posting engine, Essential / Standard / Advanced tiers, country packages) compared with the code. **The engine is further along than "basic" suggests**: a real double-entry ledger, no edit after posting, mirror reversals, month locks, automatic posting of invoices, expenses, bills, both payment kinds, credit notes and payroll, a bill approval chain and the three-way-match payment hold. **What is wrong is that the books are incomplete and the sub-sections are one level deep.** Found: (1) money that moves and never reaches the ledger — depreciation, asset purchases and disposals, the withheld part of an invoice (its receivable never clears), cheques and deal-level payments; (2) a foreign-currency bill posts its raw amount into the studio-currency book (`money.md`); (3) `statements.ts` rounds at two places, so a JOD/KWD/BHD/OMR studio's P&L and balance sheet lose the third decimal the ledger kept; (4) input and output VAT share account 2100; (5) the Ledger screen only reads (no manual entry, no reversal, no chart editor), one bank account, no cash-flow statement, no year-end; (6) four of the plan's nine sub-sections do not exist (Tax, Budgets, Reporting & Close, Entities), and invoices sit under **Cash** where the plan has Receivables. **The six steps, in the owner's order:** 1 complete the books (post what moves, convert FX, fix the statements, split VAT) · 2 make the ledger usable (chart editor, manual entry, reversal button, several bank/cash accounts, transfers, credit-note screen, cheques LINKED to the invoice or bill they settle) · 3 re-file the sub-sections the Quotations/POS way (new nav keys, rows stay where they are filed) · 4 the country essentials, per the studio's country (e-invoicing adapters such as ZATCA, withholding on bills, VAT return filing, zakat where it applies) · 5 Standard (payment runs, expense claims, dunning, credit limits, statement import and match rules, budgets on the four existing line dimensions, close checklist, year-end, cash flow) · 6 Advanced (multi-entity, IFRS 15/16, allocations). **Where the plan is adapted rather than followed:** its `/api/finance/*` + `X-Company-Id` shape contradicts invariant 2 and the studio route wrapper, so its entities are kept and its routes are not; approvals extend `platform/approval` rather than moving to the Task system; multi-entity needs a company layer INSIDE a studio, which is its own design. **THERE IS NO HOME MARKET — the owner, 18/09/2026: "the studio sets the home market when the country is selected."** The plan built a Saudi package first and `withholding.md` refused ZATCA because "this product is based in Jordan"; both assumed the product has a country, and it does not. Step 4 is therefore **country essentials, per the studio's country**: e-invoicing adapters (ZATCA for a Saudi studio, and each country's own after), withholding on bills, VAT return filing, zakat where the country has it — each living in the country's definition (`shared/compliance`) and switched on by a studio choosing it. **AND IMPORTANT SETUP IS ANNOTATED FOR USERS** (same instruction): the Finance screens say what the studio has not set up — country, currency, a VAT rate to check, and the official values its country requires on its documents — why each matters, and where to set it (`modules/finance/setup.ts`, `FinanceSetupNotice`). **Cheques and deal payments are NOT posted in step 1**: each is the same money an invoice or bill payment already posts, so posting them unlinked would count it twice — they post once they name the document they settle (step 2). |
