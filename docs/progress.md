...
| **Materials import from Excel and CSV, taken from Odoo** | ⏸ WAITING (16/09/2026) — on the owner's go, and on the client's sample Odoo exports | The first evaluating client (Noura's company) runs Odoo and is testing nompany as its replacement for 3–4 months. Excel and CSV, read on the SERVER so the browser pays nothing for the library. No row limit. Odoo columns matched automatically, a hand-made file mapped by the user, a preview of the user's own rows, every row checked (types, duplicate SKUs, units, missing suppliers) and likely swaps warned about (price below cost). Suppliers go first and missing ones are offered for creation, never created silently. Every item is tagged with its import, so a whole import can be undone while nothing uses its items yet. Failed rows download with a reason column. Opening stock arrives as a movement per item, never as a stored number. |
...
| **Multi-entity and consolidation** (18/09/2026) | The last part of the Finance plan's step 6. A studio today is ONE set of books: one chart, one currency, one journal. Several legal entities need (1) the books kept apart per entity, (2) intercompany transactions that eliminate on consolidation, and (3) a consolidated P&L and balance sheet, translated when entities keep different currencies. The plan's `X-Company-Id` header contradicts invariant 2 (membership authorises, never the URL or a header). Two shapes fit the product: **(A) entities inside a studio** — an `entityId` on every journal line and document, a chart and currency per entity, consolidation a sum with intercompany eliminated; one studio, one membership, and access per entity becomes a new scope on roles. **(B) each entity its own studio, grouped** — every studio stays exactly as it is, a group links studios with the same owner, and consolidation reads each member studio's ledger for a person who is a member of all of them; nothing in a single studio changes, but group reads cross tenants. Recommended: **(B)** — it changes no existing row and keeps invariant 2 whole, because a consolidating reader must already be a member of every studio read. Waiting on the owner. |
