...
| **Country compliance: a country's official values apply by themselves once the Owner picks the country** | 🟡 SLICES A, B AND C BUILT (18/09/2026) — official-values-model and customer-documents green, tsc and strict tsc clean, eslint clean, `npm test` green; the panel and the receipt heading checked in the sandbox, a printed layout not. B: quotations, invoices and POS receipts print the official values their country marks for them, an empty one prints nothing, and a legal row repeating one is dropped. C (the owner chose the payslip over HR letters, which do not exist): one person's payslip prints from a run, carrying the official values marked `payslip` | `docs/functionality/official-values.md`. Phase 1 researched seven countries (SA, AE, JO, EG, US federal, GB, DE) field by field with sources. Slice A is the foundation: one JSON definition per country, a validator shared by screen and server (formats plus a closed set of checksums), an **Official values** panel in Studio settings showing only the selected country's fields, an all-or-nothing save with an append-only change history, and one resolver that returns a value only when **selected, filled, applicable and valid for the current country** — otherwise an empty string, never another country's value. Adding a country is adding a file. Next: **B** sales documents (invoice, quote, sales order, POS receipt), **C** payslips, HR letters and the documentation system's merge fields, **D** moving `taxProfile`, the employment pack, `COUNTRY_PRESETS` and the WPS employer id onto the definitions. |
