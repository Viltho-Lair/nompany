...
| **A studio screen paints with its data: the page answers the screen's own GET in its render** | 🟡 IN PROGRESS (23/09/2026) — Main and Sales converted, opened in the sandbox (no `/main` or `/sales` request after the page) | The owner, 23/09/2026: a section click showed two skeletons ("double loading") where `/super` shows one. Cause: the page rendered an empty screen, and the screen then fetched its own route, which resolved the studio again from scratch. `route()` now also returns `firstPayload(slug, path)`: the same handler and context builder, run inside the page's `withRequest` so the re-resolution comes from the request cache. The page hands the body down as `initial`; `useReload(load, initial)` skips the first fetch. Refused, over the 48 KiB ceiling, or thrown → no `initial`, and the screen fetches as before. **This replaces the design's view function per route** (`2026-09-06-server-rendered-first-payload-design.md`): one mechanism instead of seventy hand-extracted copies. Remaining: every other fetch-on-mount screen, one commit each. |
