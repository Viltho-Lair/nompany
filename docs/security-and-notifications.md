...
| **Write and read rights diverge on PII** (M-9) — resolved 17/09/2026 | ID and passport numbers are no longer stored; the identity-document picture that replaced them is changed only by `hr.employees.salary`, the right that shows it. |
...
*(The last two were recorded **only** in the constraint log of `.claude/agents/frontend-ui.md`, one of the ten agent briefs deleted on 06/09/2026 — disabled since 29/08/2026 and fully commented out, so they had loaded nothing for over a week while still being the sole record of these two findings. Both were verified against the code on the day they moved here, which is why they were rescued before the deletion rather than after it. The third finding in that log — "zod is a dependency but dormant" — is **no longer true**: `src/modules` and the API routes carry 18 runtime `parse`/`safeParse` calls, so it was dropped rather than migrated.)*
