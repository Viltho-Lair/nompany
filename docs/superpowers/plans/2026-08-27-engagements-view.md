...
- **Permission matrix** gains exactly one key (`engagements.view`): 102 → 103, picked up automatically by the matrix test.
...
- [ ] **Step 4: Run to verify it passes** — `ok testPermissionKey`; both `tsc` configs clean. Run the permission-matrix suite and confirm it now covers **103** keys and stays green (the matrix enumerates `ALL_PERMISSIONS`, so the new key is picked up with no edit).
...
- [ ] **Step 2: Run the FULL `npm test` alone** and confirm: the engagement-view tests execute and pass; **Gate A is 146/146 with 0 failures**; the permission matrix covers 103 keys; hop counts unregressed. Then `tsc` both configs and `npx next build`.
